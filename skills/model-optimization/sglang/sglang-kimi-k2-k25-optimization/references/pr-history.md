# sglang Kimi K2/K2.5/Linear/VL PR Diff Audit Reference
...
| 2026-04-27 | [#23381](https://github.com/sgl-project/sglang/pull/23381) | open | [AMD] Add MI355X Kimi-K2.6 tuning artifacts | `python/sglang/srt/layers/moe/moe_runner/triton_utils/configs/triton_3_6_0/E=384,N=256,device_name=AMD_Instinct_MI355X,dtype=int4_w4a16.json`, `python/sglang/srt/environ.py` |
| 2026-04-28 | [#23563](https://github.com/sgl-project/sglang/pull/23563) | open | [Cookbook] Add Kimi K2.6 speculative decoding + fix draft attention backend | `docs_new/cookbook/autoregressive/Moonshotai/Kimi-K2.6.mdx`, `docs_new/src/snippets/autoregressive/kimi-k26-deployment.jsx` |
| 2026-04-28 | [#23848](https://github.com/sgl-project/sglang/pull/23848) | open | [AMD] Add Kimi-K2.6 in nightly tests for MI30x and MI35x | `test/registered/amd/accuracy/mi30x/test_kimi_k26_eval_amd.py`, `test/registered/amd/perf/mi35x/test_kimi_k26_perf_mi35x.py` |
...
- Status/date: open / 2026-04-28
- Trace source: active open Kimi-K2.6 AMD tuning PR cited by the skill radar.
- Diff scope read: GitHub Pull Request files API returned 4 files, +126/-2; this card prioritizes the MI355X fused MoE config and loader/env changes.
- Motivation: Title: "[AMD] Add MI355X Kimi-K2.6 tuning artifacts"; model line: Kimi K2/K2.5/K2.6; category: performance/backend optimization; main diff: `python/sglang/srt/layers/moe/moe_runner/triton_utils/configs/triton_3_6_0/E=384,N=256,device_name=AMD_Instinct_MI355X,dtype=int4_w4a16.json`, `python/sglang/srt/environ.py`; adds MI355X-specific Kimi-K2.6 tuning artifacts.
- Key implementation: adds an MI355X int4_w4a16 fused-MoE Triton config, updates tuning script naming, adds environment plumbing, and adjusts DeepSeek-style weight loader behavior for this AMD/Kimi path.
- Code diff details:
  - `python/sglang/srt/layers/moe/moe_runner/triton_utils/configs/triton_3_6_0/E=384,N=256,device_name=AMD_Instinct_MI355X,dtype=int4_w4a16.json` adds the tuning config.
  - `python/sglang/srt/environ.py` and `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py` support the runtime path.
- Key code excerpts:
...
+# Kimi-K2.6 AMD tuning/runtime toggle
```

- Reviewed files:
  - runtime/config: `python/sglang/srt/layers/moe/moe_runner/triton_utils/configs/triton_3_6_0/E=384,N=256,device_name=AMD_Instinct_MI355X,dtype=int4_w4a16.json`, `python/sglang/srt/environ.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `benchmark/kernels/fused_moe_triton/tuning_fused_moe_triton.py`
- Risk and verification: This PR is still open; do not treat the MI355X tuning config as landed until it merges. Validate decode small-M performance and weight-loading compatibility on MI355X before copying values.
...
- Status/date: open / 2026-04-28
- Trace source: active open Kimi-K2.6 speculative-decoding cookbook PR cited by the skill radar.
- Diff scope read: GitHub Pull Request files API returned 4 files, +60/-3; this card covers the docs and command-generator diff.
- Motivation: Title: "[Cookbook] Add Kimi K2.6 speculative decoding + fix draft attention backend"; model line: Kimi K2/K2.5/K2.6; category: docs/tests/CI; main diff: `docs_new/cookbook/autoregressive/Moonshotai/Kimi-K2.6.mdx`, `docs_new/src/snippets/autoregressive/kimi-k26-deployment.jsx`; adds EAGLE speculative-decoding commands and draft attention backend fixes.
- Key implementation: adds `--speculative-draft-attention-backend trtllm_mha` to Kimi-K2.5 examples; adds a Kimi-K2.6 speculative decoding section; wires a speculative selector in the Kimi-K2.6 command generator; limits K2.6 speculative decoding to H200/B300 in the generator.
- Code diff details:
  - `docs_new/cookbook/autoregressive/Moonshotai/Kimi-K2.6.mdx` adds speculative-decoding documentation.
  - `docs_new/src/snippets/autoregressive/kimi-k26-deployment.jsx` adds the UI option, env var, and EAGLE command flags.
- Key code excerpts:
...
- Reviewed files:
  - docs: `docs_new/cookbook/autoregressive/Moonshotai/Kimi-K2.5.mdx`, `docs_new/cookbook/autoregressive/Moonshotai/Kimi-K2.6.mdx`, `docs_new/src/snippets/autoregressive/kimi-k25-deployment.jsx`, `docs_new/src/snippets/autoregressive/kimi-k26-deployment.jsx`
- Risk and verification: This PR is still open; generated speculative commands should be considered proposed until merged and should be validated on H200/B300 with the draft model.

### PR #23848 - [AMD] Add Kimi-K2.6 in nightly tests for MI30x and MI35x

- Link: https://github.com/sgl-project/sglang/pull/23848
- Status/date: open / 2026-04-28
- Trace source: active open Kimi-K2.6 AMD nightly-test PR cited by the skill radar.
- Diff scope read: GitHub Pull Request files API returned 6 files, +546/-28; this card prioritizes the new AMD accuracy/performance test files and workflow matrix edits.
- Motivation: Title: "[AMD] Add Kimi-K2.6 in nightly tests for MI30x and MI35x"; model line: Kimi K2/K2.5/K2.6; category: docs/tests/CI; main diff: `test/registered/amd/accuracy/mi30x/test_kimi_k26_eval_amd.py`, `test/registered/amd/perf/mi35x/test_kimi_k26_perf_mi35x.py`; adds Kimi-K2.6 AMD nightly coverage.
- Key implementation: adds MI30x and MI35x Kimi-K2.6 accuracy/performance tests and updates AMD nightly workflows so the new tests can be dispatched.
- Code diff details:
  - `test/registered/amd/accuracy/mi30x/test_kimi_k26_eval_amd.py` and `test/registered/amd/accuracy/mi35x/test_kimi_k26_eval_mi35x.py` add evaluation lanes.
  - `test/registered/amd/perf/mi30x/test_kimi_k26_perf_amd.py` and `test/registered/amd/perf/mi35x/test_kimi_k26_perf_mi35x.py` add performance lanes.
  - AMD workflow files update the nightly dispatch matrix.
- Key code excerpts:

```diff
diff -- test/registered/amd/accuracy/mi30x/test_kimi_k26_eval_amd.py
+class TestKimiK26EvalAMD(CustomTestCase):
+    def test_kimi_k26_eval(self):
diff -- test/registered/amd/perf/mi35x/test_kimi_k26_perf_mi35x.py
+class TestKimiK26PerfMI35X(CustomTestCase):
```

- Reviewed files:
  - tests/workflows: `.github/workflows/nightly-test-amd.yml`, `.github/workflows/nightly-test-amd-rocm720.yml`, `test/registered/amd/accuracy/mi30x/test_kimi_k26_eval_amd.py`, `test/registered/amd/accuracy/mi35x/test_kimi_k26_eval_mi35x.py`, `test/registered/amd/perf/mi30x/test_kimi_k26_perf_amd.py`, `test/registered/amd/perf/mi35x/test_kimi_k26_perf_mi35x.py`
- Risk and verification: This PR is still open; treat the nightly coverage as proposed and check actual CI availability before relying on it as a required validation lane.
