...
| `docs_new/cookbook/autoregressive/DeepSeek/DeepSeek-R1.mdx` | 无直接 PR 号提交 |
| `docs_new/cookbook/autoregressive/DeepSeek/DeepSeek-V3.mdx` | 无直接 PR 号提交 |
| `docs_new/cookbook/autoregressive/DeepSeek/DeepSeek-V3_1.mdx` | 无直接 PR 号提交 |
| `docs_new/cookbook/autoregressive/DeepSeek/DeepSeek-V3_2.mdx` | 无直接 PR 号提交 |
| `docs_new/docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.mdx` | 无直接 PR 号提交 |
| `docs_new/src/snippets/autoregressive/deepseek-r1-advanced-deployment.jsx` | 无直接 PR 号提交 |
| `docs_new/src/snippets/autoregressive/deepseek-r1-basic-deployment.jsx` | 无直接 PR 号提交 |
| `docs_new/src/snippets/autoregressive/deepseek-v3-deployment.jsx` | 无直接 PR 号提交 |
| `docs_new/src/snippets/autoregressive/deepseek-v31-deployment.jsx` | 无直接 PR 号提交 |
| `docs_new/src/snippets/autoregressive/deepseek-v32-deployment.jsx` | 无直接 PR 号提交 |
...
| `python/sglang/srt/layers/attention/nsa/nsa_backend_mtp_precompute.py` | 无直接 PR 号提交 |
...
| `python/sglang/srt/layers/attention/nsa/nsa_mtp_verification.py` | 无直接 PR 号提交 |
...
| `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_kv_fp8_eval_mi35x.py` | 无直接 PR 号提交 |
...
| `test/registered/amd/test_deepseek_r1_mxfp4_8gpu.py` | 无直接 PR 号提交 |
| `test/registered/backends/test_deepseek_r1_fp8_trtllm_backend.py` | 无直接 PR 号提交 |
...
- 链接: https://github.com/sgl-project/sglang/pull/7
- 状态/时间: merged / 2024-01-15
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-2，可读 patch 15 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「fix radix cache match」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/managers/router/radix_cache.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/managers/router/radix_cache.py` modified +2/-2 (4 lines); hunks: -116,12 +116,12 @@ def _match_prefix_helper(self, node, key, value, last_node):; symbols: _match_prefix_helper，涉及 `_match_prefix_helper`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/managers/router/radix_cache.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #2637 - AMD: set weights and scaling numbers properly for block FP8

- 链接: https://github.com/sgl-project/sglang/pull/2637
- 状态/时间: merged / 2024-12-29
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+56/-6，可读 patch 120 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「AMD: set weights and scaling numbers properly for block FP8」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`；PR 正文摘要: As it is。
- 实现要点: `python/sglang/srt/layers/quantization/fp8.py` modified +38/-1 (39 lines); hunks: -272,6 +272,19 @@ def create_weights(; -369,7 +382,7 @@ def apply(; symbols: create_weights, process_weights_after_loading, apply，涉及 `create_weights, process_weights_after_loading, apply`；`python/sglang/srt/layers/quantization/fp8_kernel.py` modified +10/-3 (13 lines); hunks: -22,7 +22,10; -73,7 +76,7 @@ def per_token_group_quant_fp8(; symbols: per_token_group_quant_fp8，涉及 `per_token_group_quant_fp8`；`python/sglang/srt/layers/quantization/fp8_utils.py` modified +8/-2 (10 lines); hunks: -7,6 +7,9; -63,8 +66,11 @@ def input_to_float8(; symbols: normalize_e4m3fn_to_e4m3fnuz, input_to_float8，涉及 `normalize_e4m3fn_to_e4m3fnuz, input_to_float8`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/quantization/fp8.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #2667 - AMD DeepSeek_V3 FP8 Numerical fix

- 链接: https://github.com/sgl-project/sglang/pull/2667
- 状态/时间: merged / 2024-12-30
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c5210dfa3802`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+34/-7，可读 patch 88 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「AMD DeepSeek_V3 FP8 Numerical fix」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Together with core changes from: https://github.com/sgl-project/sglang/pull/2637 Several places in deepseek_v2.py needs extra normalization to weights (to e4m3fnuz), scaling num...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +34/-7 (41 lines); hunks: -46,6 +46,7; -55,7 +56,9; symbols: forward_absorb, load_weights, DeepseekV3ForCausalLM，涉及 `forward_absorb, load_weights, DeepseekV3ForCausalLM`。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-01-03
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+9/-5，可读 patch 58 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Feature, Hardware] Enable DeepseekV3 on AMD GPUs」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/layers/attention/triton_ops/decode_attention.py`；PR 正文摘要: - Support DeepseekV3 on AMD Instinct MI300X GPU - Add proper fix for AMD FP8 to support DeepseekV3 FP8 model - Bypass to cast FP8 to BF16 in MLA - Add AMD config。
- 实现要点: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py` modified +5/-5 (10 lines); hunks: -477,9 +477,9 @@ def invoke_fused_moe_kernel(; -614,7 +614,7 @@ def get_default_config(; symbols: invoke_fused_moe_kernel, get_default_config, fused_experts_impl，涉及 `invoke_fused_moe_kernel, get_default_config, fused_experts_impl`；`python/sglang/srt/layers/attention/triton_ops/decode_attention.py` modified +4/-0 (4 lines); hunks: -406,6 +406,10 @@ def _decode_grouped_att_m_fwd(; symbols: _decode_grouped_att_m_fwd，涉及 `_decode_grouped_att_m_fwd`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/attention/triton_ops/decode_attention.py`, `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #3314 - Feature/docs deepseek usage and add multi-node

- 链接: https://github.com/sgl-project/sglang/pull/3314
- 状态/时间: merged / 2025-02-07
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 0 个文件，+0/-0，可读 patch 0 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Feature/docs deepseek usage and add multi-node」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: 文件级 diff；PR 正文摘要: People are using the Deepseek V3 model, our guidance are not centralised for the ease of usage. Update: docs\references\deepseek.md Update: docs\references\multi_node.md Remove:...。
...
- 链接: https://github.com/sgl-project/sglang/pull/3466
- 状态/时间: merged / 2025-02-10
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+286/-42，可读 patch 513 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support Eagle2 for Triton backend」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/triton_backend.py`, `python/sglang/srt/layers/attention/triton_ops/extend_attention.py`, `python/sglang/srt/speculative/eagle_worker.py`；PR 正文摘要: Support Eagle2 for Triton backend and achieve **2.6x** speedup on batch size 1 for no cuda graph setting. (Based on https://github.com/sgl-project/sglang/pull/3317, https://gith...。
- 实现要点: `python/sglang/srt/layers/attention/triton_backend.py` modified +223/-24 (247 lines); hunks: -3,6 +3,7; -18,7 +19,12; symbols: TritonAttnBackend, __init__，涉及 `TritonAttnBackend, __init__`；`python/sglang/srt/layers/attention/triton_ops/extend_attention.py` modified +4/-4 (8 lines); hunks: -50,7 +50,7 @@ def _fwd_kernel(; -87,7 +87,7 @@ def _fwd_kernel(; symbols: _fwd_kernel, extend_attention_fwd，涉及 `_fwd_kernel, extend_attention_fwd`；`python/sglang/srt/speculative/eagle_worker.py` modified +24/-8 (32 lines); hunks: -65,15 +65,31 @@ def __init__(; symbols: __init__，涉及 `__init__`；`test/srt/test_eagle_infer.py` modified +29/-0 (29 lines); hunks: -193,5 +193,34 @@ def test_gsm8k(self):; symbols: test_gsm8k, TestEAGLEServerTriton, setUpClass，涉及 `test_gsm8k, TestEAGLEServerTriton, setUpClass`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/test_eagle_infer.py`, `test/srt/test_triton_attention_kernels.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+7/-0，可读 patch 35 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「refine deepseek_v3 launch server doc」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `benchmark/deepseek_v3/README.md`；PR 正文摘要: RT。
- 实现要点: `benchmark/deepseek_v3/README.md` modified +7/-0 (7 lines); hunks: -41,6 +41,7 @@ python3 -m sglang.launch_server --model deepseek-ai/DeepSeek-V...; -82,6 +83,8 @@ python3 -m sglang.launch_server --model-path deepseek-ai/DeepS...。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/3550
- 状态/时间: merged / 2025-02-14
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `70f894b810c0`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 12 个文件，+296/-132，可读 patch 737 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「feat: support flashinfer mla attention for deepseek v3」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Kudos to @yzh119 Throughout the integration process, we have identified and resolved numerous issues with the exceptional support from the FlashInfer team. Currently, **SGLang i...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +13/-7 (20 lines); hunks: -510,14 +510,20 @@ def forward(; symbols: forward, forward_normal，涉及 `forward, forward_normal`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/test_eagle_infer.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #3582 - Support NextN (MTP) speculative decoding for DeepSeek-V3/R1

- 链接: https://github.com/sgl-project/sglang/pull/3582
- 状态/时间: merged / 2025-02-14
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `862dd76c7619`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+437/-7，可读 patch 529 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support NextN (MTP) speculative decoding for DeepSeek-V3/R1」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: We implemented NextN (MTP) speculative decoding for DeepSeek-V3/R1 based on EAGLE 2 on Triton backend (https://github.com/sgl-project/sglang/pull/3466) and achieved **1.76x** sp...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +4/-1 (5 lines); hunks: -519,6 +519,8 @@ def forward(; -680,6 +682,7 @@ def __init__(; symbols: forward, __init__，涉及 `forward, __init__`。
- 代码 diff 细节:
...
- 动机: 标题「Refactor flashinfer logic for deepseek v3 and fix accuracy bug」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: `flashinfer_backend.py` for attention is too complex, this PR extract the logic of MLA and creates a new `flashinfer_mla_backend.py` Also, #3716 #3751 reports an accuracy bug wh...。
...
- 状态/时间: merged / 2025-02-24
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `1a6e97577acb`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+1092/-5，可读 patch 1339 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Feature DeepSeek V3/R1 INT8 Quantization (block-wise)」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Support block-wise INT8 quantization for DeepSeek V3/R1. INT8 is a friendly type for most hardware platforms. - Fork FP8 implementation and support the block-wise quantization o...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +15/-0 (15 lines); hunks: -47,6 +47,9; -994,6 +997,18 @@ def load_weights(self, weights: Iterable[Tuple[str, torch.T...; symbols: load_weights，涉及 `load_weights`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/3237
- 状态/时间: merged / 2025-02-25
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `6ce9dbe82882`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+863/-1，可读 patch 894 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[ROCm] Enable Fused MLA Triton kernel for DeepSeekV3」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This PR introduces the concept of fusing MLA rope into the grouped attention on ROCm. To use this feature use the env variable : SGLANG_ROCM_FUSED_DECODE_MLA=1. Triton Kernel au...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +159/-1 (160 lines); hunks: -16,6 +16,7; -31,6 +32,9; symbols: no_absorb, forward_normal, forward_absorb, forward_absorb_fused_mla_rope，涉及 `no_absorb, forward_normal, forward_absorb`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/test_triton_attention_rocm_mla.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #3893 - add deepgemm and sglang fp8 block-wise gemm benchmark

- 链接: https://github.com/sgl-project/sglang/pull/3893
- 状态/时间: merged / 2025-03-02
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+320/-0，可读 patch 322 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「add deepgemm and sglang fp8 block-wise gemm benchmark」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `benchmark/kernels/deepseek/benchmark_deepgemm_fp8_gemm.py`, `benchmark/kernels/deepseek/README.md`；PR 正文摘要: The script benchmark all FP8 w8a8 block-wise matrix multiplications involved in DeepSeek V3/R1 under the current tensor parallelism (TP) setting with DeepGemm and vLLM/SGLang. *...。
- 实现要点: `benchmark/kernels/deepseek/benchmark_deepgemm_fp8_gemm.py` added +314/-0 (314 lines); hunks: -0,0 +1,314; symbols: per_token_cast_to_fp8, per_block_cast_to_fp8, fp8_gemm_deepgemm, fp8_gemm_sglang，涉及 `per_token_cast_to_fp8, per_block_cast_to_fp8, fp8_gemm_deepgemm`；`benchmark/kernels/deepseek/README.md` added +6/-0 (6 lines); hunks: -0,0 +1,6。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/3888
- 状态/时间: merged / 2025-03-07
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c7f254468fca`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+369/-21，可读 patch 503 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Feature] DeepSeek V3/R1 INT8 Quantization (channel-wise)」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Support channel-wise INT8 quantization for DeepSeek V3/R1. INT8 is a friendly type for most hardware platforms. Co-author: @yych0745 @sleepcoo @b0urnee - Moe: Fused moe triton k...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +16/-12 (28 lines); hunks: -1202,18 +1202,22 @@ def load_weights(self, weights: Iterable[Tuple[str, torc...; symbols: load_weights，涉及 `load_weights`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/run_suite.py`, `test/srt/test_int8_kernel.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #4218 - Support nextn for flashinfer mla attention backend

- 链接: https://github.com/sgl-project/sglang/pull/4218
- 状态/时间: merged / 2025-03-09
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+393/-58，可读 patch 648 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support nextn for flashinfer mla attention backend」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/srt/test_mla_flashinfer.py`；PR 正文摘要: Support the compatibility of nextn and flashinfer mla attention backend. Currently topk can only be set to 1 due to lack of custom mask support for flashinfer MLA wrapper. - Imp...。
- 实现要点: `python/sglang/srt/layers/attention/flashinfer_mla_backend.py` modified +317/-57 (374 lines); hunks: -11,9 +11,10; -23,6 +24,7; symbols: FlashInferMLAAttnBackend, __init__, init_forward_metadata，涉及 `FlashInferMLAAttnBackend, __init__, init_forward_metadata`；`python/sglang/srt/models/deepseek_v2.py` modified +2/-0 (2 lines); hunks: -555,6 +555,8 @@ def no_absorb() -> bool:; symbols: no_absorb，涉及 `no_absorb`；`test/srt/test_mla_flashinfer.py` modified +63/-0 (63 lines); hunks: -1,6 +1,7; -100,5 +101,67 @@ def test_gsm8k(self):; symbols: test_gsm8k, TestFlashinferMLAMTP, setUpClass, tearDownClass，涉及 `test_gsm8k, TestFlashinferMLAMTP, setUpClass`；`python/sglang/srt/speculative/eagle_worker.py` modified +10/-0 (10 lines); hunks: -123,6 +123,16 @@ def init_attention_backend(self):; symbols: init_attention_backend，涉及 `init_attention_backend`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/test_mla_flashinfer.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #4165 - DeepGemm integrate to sgl-kernel

- 链接: https://github.com/sgl-project/sglang/pull/4165
- 状态/时间: merged / 2025-03-10
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+324/-5，可读 patch 388 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「DeepGemm integrate to sgl-kernel」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `sgl-kernel/tests/test_deep_gemm.py`, `sgl-kernel/setup.py`, `sgl-kernel/build.sh`；PR 正文摘要: Integrate DeepGemm in setup. Linear usage: #4199 .。
- 实现要点: `sgl-kernel/tests/test_deep_gemm.py` added +263/-0 (263 lines); hunks: -0,0 +1,263; symbols: per_token_cast_to_fp8, per_block_cast_to_fp8, construct, construct_grouped，涉及 `per_token_cast_to_fp8, per_block_cast_to_fp8, construct`；`sgl-kernel/setup.py` modified +52/-1 (53 lines); hunks: -14,11 +14,13; -52,6 +54,7 @@ def _get_version():; symbols: _get_version, CustomBuildPy, run, make_jit_include_symlinks，涉及 `_get_version, CustomBuildPy, run`；`sgl-kernel/build.sh` modified +4/-3 (7 lines); hunks: -11,11 +11,11 @@ else; -24,5 +24,6 @@ docker run --rm \；`.gitmodules` modified +3/-0 (3 lines); hunks: -7,3 +7,6。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `sgl-kernel/tests/test_deep_gemm.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+76/-44，可读 patch 184 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「linear support deepgemm」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/test/test_block_fp8.py`, `test/srt/test_fp8_kernel.py`；PR 正文摘要: Use deepgemm linear gemm, dependent on https://github.com/sgl-project/sglang/pull/4165 Test case **MMLU Average accuracy: 0.878** performence test size | origin | deepgemm linea...。
- 实现要点: `python/sglang/srt/layers/quantization/fp8_kernel.py` modified +36/-28 (64 lines); hunks: -29,10 +29,13; -722,34 +725,39 @@ def grid(META):; symbols: _per_token_group_quant_fp8, grid，涉及 `_per_token_group_quant_fp8, grid`；`python/sglang/test/test_block_fp8.py` modified +39/-15 (54 lines); hunks: -1,4 +1,5; -11,6 +12,8; symbols: native_per_token_group_quant_fp8, native_w8a8_block_fp8_matmul, TestW8A8BlockFP8Matmul, setUpClass，涉及 `native_per_token_group_quant_fp8, native_w8a8_block_fp8_matmul, TestW8A8BlockFP8Matmul`；`test/srt/test_fp8_kernel.py` modified +1/-1 (2 lines); hunks: -17,7 +17,7 @@ def setUpClass(cls):; symbols: setUpClass, _make_A，涉及 `setUpClass, _make_A`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `python/sglang/test/test_block_fp8.py`, `test/srt/test_fp8_kernel.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+16/-2，可读 patch 42 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「add INT8 example into dsv3 README」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `benchmark/deepseek_v3/README.md`；PR 正文摘要: add INT8 example into benchmark/deepseek_v3/README.md。
- 实现要点: `benchmark/deepseek_v3/README.md` modified +16/-2 (18 lines); hunks: -184,25 +184,39 @@ AWQ does not support BF16, so add the `--dtype half` flag...。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-03-16
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+209/-1，可读 patch 285 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support FlashMLA backend」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/model_executor/model_runner.py`；PR 正文摘要: Integrate flashmla for decoding, and the accuracy test is currently okay. The current implementation is quite simple, directly integrating flashmla as the backend. Later, we nee...。
- 实现要点: `python/sglang/srt/layers/attention/flashmla_backend.py` added +128/-0 (128 lines); hunks: -0,0 +1,128; symbols: FlashMLABackend, __init__, forward_decode，涉及 `FlashMLABackend, __init__, forward_decode`；`python/sglang/srt/layers/attention/utils.py` modified +54/-0 (54 lines); hunks: -15,6 +15,7 @@ def create_flashinfer_kv_indices_triton(; -37,3 +38,56 @@ def create_flashinfer_kv_indices_triton(; symbols: create_flashinfer_kv_indices_triton, create_flashmla_kv_indices_triton，涉及 `create_flashinfer_kv_indices_triton, create_flashmla_kv_indices_triton`；`python/sglang/srt/model_executor/model_runner.py` modified +8/-0 (8 lines); hunks: -149,6 +149,7 @@ def __init__(; -223,6 +224,9 @@ def model_specific_adjustment(self):; symbols: __init__, model_specific_adjustment, init_attention_backend，涉及 `__init__, model_specific_adjustment, init_attention_backend`；`python/sglang/srt/server_args.py` modified +8/-0 (8 lines); hunks: -173,6 +173,7 @@ class ServerArgs:; -227,6 +228,8 @@ def __post_init__(self):; symbols: ServerArgs, __post_init__, add_cli_args，涉及 `ServerArgs, __post_init__, add_cli_args`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/managers/schedule_batch.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #4514 - Support FlashMLA backend cuda graph

- 链接: https://github.com/sgl-project/sglang/pull/4514
- 状态/时间: merged / 2025-03-19
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+188/-32，可读 patch 286 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support FlashMLA backend cuda graph」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/server_args.py`；PR 正文摘要: Support FlashMLA backend cuda graph. Optimize index calculation, complete the calculation in init_forward * Optimize the FlashInfer block table calculation logic to compute only...。
- 实现要点: `python/sglang/srt/layers/attention/flashmla_backend.py` modified +184/-30 (214 lines); hunks: -1,16 +1,13; -28,10 +25,30; symbols: FlashMLADecodeMetadata, __init__, FlashMLABackend，涉及 `FlashMLADecodeMetadata, __init__, FlashMLABackend`；`python/sglang/srt/layers/attention/utils.py` modified +0/-1 (1 lines); hunks: -49,7 +49,6 @@ def create_flashmla_kv_indices_triton(; symbols: create_flashmla_kv_indices_triton，涉及 `create_flashmla_kv_indices_triton`；`python/sglang/srt/server_args.py` modified +4/-1 (5 lines); hunks: -232,7 +232,10 @@ def __post_init__(self):; symbols: __post_init__，涉及 `__post_init__`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/server_args.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+16/-3，可读 patch 53 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Set deepgemm to the default value in the hopper architecture.」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/utils.py`；PR 正文摘要: Set deepgemm to the default value in the hopper architecture.。
- 实现要点: `python/sglang/srt/layers/quantization/fp8_kernel.py` modified +9/-3 (12 lines); hunks: -26,11 +26,14; -39,9 +42,12; symbols: grid，涉及 `grid`；`python/sglang/srt/utils.py` modified +7/-0 (7 lines); hunks: -1006,6 +1006,13 @@ def get_amdgpu_memory_capacity():; symbols: get_amdgpu_memory_capacity, get_device_sm, get_nvgpu_memory_capacity，涉及 `get_amdgpu_memory_capacity, get_device_sm, get_nvgpu_memory_capacity`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/utils.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #4631 - [ROCm] Enable MTP (NextN) on AMD GPU

- 链接: https://github.com/sgl-project/sglang/pull/4631
- 状态/时间: merged / 2025-03-24
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+43/-4，可读 patch 99 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[ROCm] Enable MTP (NextN) on AMD GPU」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `sgl-kernel/csrc/speculative/pytorch_extension_utils_rocm.h`, `sgl-kernel/csrc/torch_extension_rocm.cc`, `python/sglang/srt/speculative/build_eagle_tree.py`；PR 正文摘要: To enable MTP (NextN) on AMD GPU. Added a few kernels to `torch_extension_rocm.cc`. Enabled MTP test for AMD GPU. Benchmark result Correctness check。
- 实现要点: `sgl-kernel/csrc/speculative/pytorch_extension_utils_rocm.h` added +20/-0 (20 lines); hunks: -0,0 +1,20；`sgl-kernel/csrc/torch_extension_rocm.cc` modified +12/-0 (12 lines); hunks: -65,6 +65,18 @@ TORCH_LIBRARY_EXPAND(sgl_kernel, m) {；`python/sglang/srt/speculative/build_eagle_tree.py` modified +2/-2 (4 lines); hunks: -4,9 +4,9；`python/sglang/srt/speculative/eagle_utils.py` modified +3/-1 (4 lines); hunks: -14,7 +14,7; -23,6 +23,8。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-03-29
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+616/-1，可读 patch 659 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Add deepseek style fused moe group gate selection kernel」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `sgl-kernel/csrc/moe/moe_fused_gate.cu`, `sgl-kernel/benchmark/bench_moe_fused_gate.py`, `sgl-kernel/tests/test_moe_fused_gate.py`；PR 正文摘要: PR adapted and improved from #3191 Rewrite Macro. Extended to support all power of 2 `# expert` & `# expert group`, also all `# topk_group` & `# topk` use cases + dtype support...。
- 实现要点: `sgl-kernel/csrc/moe/moe_fused_gate.cu` added +447/-0 (447 lines); hunks: -0,0 +1,447；`sgl-kernel/benchmark/bench_moe_fused_gate.py` added +74/-0 (74 lines); hunks: -0,0 +1,74; symbols: biased_grouped_topk_org, biased_grouped_topk_org_kernel, benchmark，涉及 `biased_grouped_topk_org, biased_grouped_topk_org_kernel, benchmark`；`sgl-kernel/tests/test_moe_fused_gate.py` added +72/-0 (72 lines); hunks: -0,0 +1,72; symbols: test_moe_fused_gate_combined，涉及 `test_moe_fused_gate_combined`；`sgl-kernel/python/sgl_kernel/moe.py` modified +12/-0 (12 lines); hunks: -32,3 +32,15 @@ def topk_softmax(; symbols: topk_softmax, moe_fused_gate，涉及 `topk_softmax, moe_fused_gate`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/4817
- 状态/时间: merged / 2025-04-03
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+252/-80，可读 patch 489 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[sgl-kernel] per token group quant support COLUMN MAJOR」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `sgl-kernel/tests/test_per_token_group_quant_8bit.py`, `sgl-kernel/csrc/gemm/per_token_group_quant_8bit.cu`, `sgl-kernel/benchmark/bench_per_token_group_quant_8bit.py`；PR 正文未提供可用摘要。
- 实现要点: `sgl-kernel/tests/test_per_token_group_quant_8bit.py` modified +196/-58 (254 lines); hunks: -9,12 +9,12; -25,15 +25,16 @@ def _per_token_group_quant_8bit(; symbols: _per_token_group_quant_8bit, _per_token_group_quant_fp8, _per_token_group_quant_fp8_colmajor，涉及 `_per_token_group_quant_8bit, _per_token_group_quant_fp8, _per_token_group_quant_fp8_colmajor`；`sgl-kernel/csrc/gemm/per_token_group_quant_8bit.cu` modified +49/-19 (68 lines); hunks: -16,7 +16,7 @@ __device__ __forceinline__ float GroupReduceMax(float val, con...; -26,19 +26,30 @@ __global__ void per_token_group_quant_8bit_kernel(；`sgl-kernel/benchmark/bench_per_token_group_quant_8bit.py` modified +7/-3 (10 lines); hunks: -148,9 +148,11 @@ def sglang_per_token_group_quant_8bit(; -196,7 +198,9 @@ def benchmark(batch_size, seq_len, group_size, dst_dtype, pr...; symbols: sglang_per_token_group_quant_8bit, calculate_diff, benchmark，涉及 `sglang_per_token_group_quant_8bit, calculate_diff, benchmark`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `sgl-kernel/tests/test_per_token_group_quant_8bit.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #4918 - Add DeepSeek V3/R1 shared experts fusion

- 链接: https://github.com/sgl-project/sglang/pull/4918
- 状态/时间: merged / 2025-04-04
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `924ca7c92c86`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 14 个文件，+536/-36，可读 patch 939 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Add DeepSeek V3/R1 shared experts fusion」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: I gain idea mainly from https://github.com/vllm-project/vllm/pull/15502 , thanks for the author's work.I will add references in the modifications for `grouped_topk` function and...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +81/-5 (86 lines); hunks: -16,12 +16,14; -87,6 +89,8; symbols: DeepseekV2MLP, __init__，涉及 `DeepseekV2MLP, __init__`。
- 代码 diff 细节:
...
- 动机: 标题「Fix DeepSeek error when using DeepEP mode」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: command output: it does not error, and gsm8k is 0.936。
...
- 状态/时间: merged / 2025-04-10
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+143/-56，可读 patch 263 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「reduce moe_align_block_size_kernel small batch mode overhead」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `sgl-kernel/csrc/moe/moe_align_kernel.cu`, `sgl-kernel/benchmark/bench_moe_align_block_size.py`；PR 正文摘要: Acc test I set `token_cnts_buffer` and `cumsum_buffer` to `torch.empty` in `fused_moe.py`: Acc result: Kernel unit-test !图片 Benchmark In H200 main branch: pr:。
- 实现要点: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py` modified +1/-1 (2 lines); hunks: -702,7 +702,7 @@ def moe_align_block_size(; symbols: moe_align_block_size，涉及 `moe_align_block_size`；`sgl-kernel/csrc/moe/moe_align_kernel.cu` modified +111/-44 (155 lines); hunks: -64,10 +64,10 @@ __global__ void moe_align_block_size_kernel(; -98,6 +98,65 @@ __global__ void moe_align_block_size_kernel(；`sgl-kernel/benchmark/bench_moe_align_block_size.py` modified +31/-10 (41 lines); hunks: -241,9 +241,9 @@ def calculate_diff(num_tokens, num_experts=256, block_size=1...; -294,17 +294,28 @@ def benchmark(num_tokens, num_experts, topk, provider):; symbols: calculate_diff, benchmark, sgl_moe_align_block_size_with_empty，涉及 `calculate_diff, benchmark, sgl_moe_align_block_size_with_empty`；`sgl-kernel/tests/test_moe_align.py` modified +0/-1 (1 lines); hunks: -151,7 +151,6 @@ def moe_align_block_size_triton(; symbols: moe_align_block_size_triton, test_moe_align_block_size_compare_implementations，涉及 `moe_align_block_size_triton, test_moe_align_block_size_compare_implementations`。
- 代码 diff 细节:
...
- 动机: 标题「fix: use deepgemm only on hopper」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/quantization/fp8_kernel.py`；PR 正文未提供可用摘要。
...
- 状态/时间: merged / 2025-04-12
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+42/-11，可读 patch 121 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「feat: use fa3 mla by default on hopper」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/utils.py`；PR 正文摘要: - Support DP MLA for FA3。
- 实现要点: `python/sglang/srt/model_executor/model_runner.py` modified +21/-4 (25 lines); hunks: -80,6 +80,7; -245,7 +246,16 @@ def model_specific_adjustment(self):; symbols: model_specific_adjustment, init_attention_backend，涉及 `model_specific_adjustment, init_attention_backend`；`python/sglang/srt/layers/attention/flashattention_backend.py` modified +12/-7 (19 lines); hunks: -325,7 +325,7 @@ def init_forward_metadata(self, forward_batch: ForwardBatch):; -527,7 +527,9 @@ def forward_extend(; symbols: init_forward_metadata, forward_extend, forward_decode, init_forward_metadata_capture_cuda_graph，涉及 `init_forward_metadata, forward_extend, forward_decode`；`python/sglang/srt/utils.py` modified +9/-0 (9 lines); hunks: -1828,3 +1828,12 @@ def fast_topk(values, topk, dim):; symbols: fast_topk, is_hopper_with_cuda_12_3，涉及 `fast_topk, is_hopper_with_cuda_12_3`。
- 代码 diff 细节:
...
- 动机: 标题「Apply deepseek cuda rope」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/rotary_embedding.py`；PR 正文摘要: The torch.compile issue described in #4959 arises from overriding the `forward` method of `CustomOp`. When torch.compile is applied, the `_forward_method` of `CustomOp` is repla...。
...
- 状态/时间: merged / 2025-04-14
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+37/-16，可读 patch 82 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「apply fused moe gate in ds v3/r1」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/topk.py`；PR 正文摘要: torch profile main branch !图片 pr !图片 Only one kernel now. 36us->8us. fused_moe_gate gsm8k acc test fused moe gate performance(in H200) |qps|Input token throughput (tok/s)|Output...。
- 实现要点: `python/sglang/srt/layers/moe/topk.py` modified +37/-16 (53 lines); hunks: -12,6 +12,7; -25,6 +26,8; symbols: biased_grouped_topk_impl, is_power_of_two, biased_grouped_topk, select_experts，涉及 `biased_grouped_topk_impl, is_power_of_two, biased_grouped_topk`。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-04-15
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+6/-2，可读 patch 22 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Fix] Turn off DeepGEMM by default」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/quantization/fp8_kernel.py`, `docs/references/deepseek.md`；PR 正文摘要: Currently DeepGEMM is turned on by default for deepseek models, which caused confusions (#5223) on its performance due to warmup time. This PR makes DeepGEMM turned off by defau...。
- 实现要点: `python/sglang/srt/layers/quantization/fp8_kernel.py` modified +3/-1 (4 lines); hunks: -45,7 +45,9；`docs/references/deepseek.md` modified +3/-1 (4 lines); hunks: -136,7 +136,9 @@ With data parallelism attention enabled, we have achieved up...。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-04-15
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+638/-4，可读 patch 690 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「kernel: support slightly faster merge_state_v2 cuda kernel」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `sgl-kernel/tests/test_merge_state_v2.py`, `sgl-kernel/csrc/attention/merge_attn_states.cu`, `sgl-kernel/python/sgl_kernel/attention.py`；PR 正文摘要: support slightly faster merge_state_v2 cuda kernel Performance 0.0000 means headsize is not supported. | tokens | heads | headsize | dtype | device | torch | triton | v1 | v2 |...。
- 实现要点: `sgl-kernel/tests/test_merge_state_v2.py` added +396/-0 (396 lines); hunks: -0,0 +1,396; symbols: merge_state_kernel, merge_state_triton, merge_state_torch, generate_markdown_table，涉及 `merge_state_kernel, merge_state_triton, merge_state_torch`；`sgl-kernel/csrc/attention/merge_attn_states.cu` added +201/-0 (201 lines); hunks: -0,0 +1,201；`sgl-kernel/python/sgl_kernel/attention.py` modified +35/-4 (39 lines); hunks: -1,4 +1,4; -10,16 +10,47 @@ def lightning_attention_decode(q, k, v, past_kv, slope, outp...; symbols: lightning_attention_decode, merge_state, merge_state_v2, cutlass_mla_decode，涉及 `lightning_attention_decode, merge_state, merge_state_v2`；`sgl-kernel/csrc/common_extension.cc` modified +2/-0 (2 lines); hunks: -47,6 +47,8 @@ TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `sgl-kernel/tests/test_merge_state_v2.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+734/-46，可读 patch 947 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support MHA with chunked prefix cache for DeepSeek chunked prefill」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/model_executor/forward_batch_info.py`, `python/sglang/srt/layers/attention/flashattention_backend.py`；PR 正文摘要: The current implementation of MLA is slow when when handling long prefix lengths, such as sequences with 32k input length, as highlighted in issues like #5031. Profiling reveale...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +174/-9 (183 lines); hunks: -18,6 +18,7; -78,7 +79,7; symbols: AttnForwardMethod, DeepseekV2MLP, __init__, no_absorb，涉及 `AttnForwardMethod, DeepseekV2MLP, __init__`；`python/sglang/srt/model_executor/forward_batch_info.py` modified +181/-0 (181 lines); hunks: -181,6 +181,28 @@ class ForwardBatch:; -484,6 +506,128 @@ def _compute_mrope_positions(; symbols: ForwardBatch, _compute_mrope_positions, get_max_chunk_capacity, set_prefix_chunk_idx，涉及 `ForwardBatch, _compute_mrope_positions, get_max_chunk_capacity`；`python/sglang/srt/layers/attention/flashattention_backend.py` modified +80/-34 (114 lines); hunks: -16,7 +16,7; -593,41 +593,87 @@ def forward_extend(; symbols: forward_extend, forward_decode，涉及 `forward_extend, forward_decode`；`python/sglang/srt/model_executor/model_runner.py` modified +11/-0 (11 lines); hunks: -167,6 +167,7 @@ def __init__(; -318,6 +319,16 @@ def model_specific_adjustment(self):; symbols: __init__, model_specific_adjustment, init_torch_distributed，涉及 `__init__, model_specific_adjustment, init_torch_distributed`。
- 代码 diff 细节:
...
- 已读文件:
  ...
  - docs: `docs/references/deepseek.md` modified +3/-1
- 验证与风险: diff 自带测试面 `python/sglang/test/attention/test_prefix_chunk_info.py`, `test/srt/test_fa3.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #5205 - Refactor DeepSeek decoder layer branches

- 链接: https://github.com/sgl-project/sglang/pull/5205
- 状态/时间: merged / 2025-04-17
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+47/-21，可读 patch 157 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Refactor DeepSeek decoder layer branches」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This code depends on https://github.com/sgl-project/sglang/pull/5190 and please subtract diff from there。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +47/-21 (68 lines); hunks: -18,7 +18,8; -28,6 +29,7; symbols: __init__, forward, forward_normal_chunked_kv, _FFNInputMode，涉及 `__init__, forward, forward_normal_chunked_kv`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/4836
- 状态/时间: merged / 2025-04-18
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `53dcf3887639`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+31/-1，可读 patch 101 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Introduce moe_dense_tp_size to fix dense layer errors in DeepSeek V3 + 4x8xH100」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This code depends on https://github.com/sgl-project/sglang/pull/5190 and https://github.com/sgl-project/sglang/pull/5205 and please subtract diff from there 4x8xH100 cannot run...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +17/-1 (18 lines); hunks: -1066,12 +1066,18 @@ def __init__(; -1084,6 +1090,10 @@ def __init__(; symbols: __init__, _enable_moe_dense_fully_dp, _compute_info，涉及 `__init__, _enable_moe_dense_fully_dp, _compute_info`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/managers/schedule_batch.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 状态/时间: merged / 2025-04-18
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+25/-6，可读 patch 52 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「use sglang_per_token_group_quant_fp8 from sgl-kernel instead of trion kernel」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`；PR 正文摘要: This pr implemented a faster kernel for per_token_group_quant_fp8, which can replace triton kernel. https://github.com/sgl-project/sglang/pull/4817 I tested the kernel with 2048...。
- 实现要点: `python/sglang/srt/layers/quantization/fp8_kernel.py` modified +24/-5 (29 lines); hunks: -275,18 +275,37 @@ def sglang_per_token_group_quant_fp8(; symbols: sglang_per_token_group_quant_fp8，涉及 `sglang_per_token_group_quant_fp8`；`python/sglang/srt/layers/quantization/fp8_utils.py` modified +1/-1 (2 lines); hunks: -141,7 +141,7 @@ def apply_w8a8_block_fp8_linear(; symbols: apply_w8a8_block_fp8_linear，涉及 `apply_w8a8_block_fp8_linear`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+62/-18，可读 patch 276 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Remove one kernel in per_tensor_quant_mla_fp8」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/models/deepseek_nextn.py`；PR 正文摘要: Thanks @Alcanderian for discussing it is acceptable to change APIs in the caller site Accuracy PR: 93.3 (baseline: roughly 93 when I tested in https://github.com/sgl-project/sgl...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +32/-9 (41 lines); hunks: -76,7 +76,7; -97,7 +97,6; symbols: AttnForwardMethod, forward, forward_normal, forward_absorb，涉及 `AttnForwardMethod, forward, forward_normal`；`python/sglang/srt/layers/quantization/fp8_kernel.py` modified +9/-7 (16 lines); hunks: -58,10 +58,8; -897,24 +895,28 @@ def _per_tensor_quant_mla_fp8_stage2(; symbols: deep_gemm_fp8_fp8_bf16_nt, _per_tensor_quant_mla_fp8_stage2, per_tensor_quant_mla_fp8，涉及 `deep_gemm_fp8_fp8_bf16_nt, _per_tensor_quant_mla_fp8_stage2, per_tensor_quant_mla_fp8`；`python/sglang/srt/models/deepseek_nextn.py` modified +8/-2 (10 lines); hunks: -40,7 +40,7; -91,6 +91,12 @@ def forward(; symbols: forward，涉及 `forward`；`python/sglang/srt/utils.py` modified +13/-0 (13 lines); hunks: -1932,3 +1932,16 @@ def is_fa3_default_architecture(hf_config):; symbols: is_fa3_default_architecture, BumpAllocator, __init__, allocate，涉及 `is_fa3_default_architecture, BumpAllocator, __init__`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+361/-20，可读 patch 481 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[perf] introduce deep gemm group_gemm_masked as bmm」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/test/test_block_fp8.py`；PR 正文摘要: per-token-group quant+deep_gemm's grouped_gemm_masked is generally faster than per-tensor quant+bmm_fp8 NOTE: Multi-batch throughput is hardly comparable because expert loads di...。
- 实现要点: `python/sglang/srt/layers/quantization/fp8_kernel.py` modified +108/-4 (112 lines); hunks: -44,6 +44,7; -53,10 +54,11; symbols: per_tensor_quant_mla_fp8, _per_token_group_quant_mla_deep_gemm_masked_fp8, per_tensor_quant_mla_deep_gemm_masked_fp8, scaled_fp8_quant，涉及 `per_tensor_quant_mla_fp8, _per_token_group_quant_mla_deep_gemm_masked_fp8, per_tensor_quant_mla_deep_gemm_masked_fp8`；`python/sglang/srt/models/deepseek_v2.py` modified +86/-16 (102 lines); hunks: -57,7 +57,11; -82,6 +86,7; symbols: __init__, forward_absorb, post_load_weights，涉及 `__init__, forward_absorb, post_load_weights`；`python/sglang/test/test_block_fp8.py` modified +167/-0 (167 lines); hunks: -7,6 +7,7; -212,6 +213,62 @@ def test_per_tensor_quant_mla_fp8(self):; symbols: test_per_tensor_quant_mla_fp8, TestPerTokenGroupQuantMlaDeepGemmMaskedFP8, setUpClass, _per_token_group_quant_mla_deep_gemm_masked_fp8，涉及 `test_per_tensor_quant_mla_fp8, TestPerTokenGroupQuantMlaDeepGemmMaskedFP8, setUpClass`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `python/sglang/test/test_block_fp8.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #5571 - enable DeepSeek V3 shared_experts_fusion in sm90

- 链接: https://github.com/sgl-project/sglang/pull/5571
- 状态/时间: merged / 2025-04-20
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `d9dd529854f7`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+12/-0，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「enable DeepSeek V3 shared_experts_fusion in sm90」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: H200 Benchmark I tested the benchmark using the command provided by https://github.com/sgl-project/sglang/issues/5514. To avoid warmup, I turned off deepgemm. Below are the resu...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +12/-0 (12 lines); hunks: -1427,6 +1427,18 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+18/-21，可读 patch 112 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Remove extra copy in deepseek forward absorb」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/rotary_embedding.py`, `.github/workflows/pr-test-amd.yml`；PR 正文摘要: Performance main branch: this PR: 0.5% improvement for bs 1 and 1.5% improvement for bs 32. main branch: this PR: removed 3 copy with 2 cat added (can overlap one later), durati...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +9/-13 (22 lines); hunks: -682,10 +682,6 @@ def forward_absorb(; -729,20 +725,20 @@ def forward_absorb(; symbols: forward_absorb，涉及 `forward_absorb`；`python/sglang/srt/layers/rotary_embedding.py` modified +2/-1 (3 lines); hunks: -665,6 +665,7 @@ def forward_native(; -695,7 +696,7 @@ def forward_native(; symbols: forward_native, Llama3RotaryEmbedding，涉及 `forward_native, Llama3RotaryEmbedding`；`.github/workflows/pr-test-amd.yml` modified +7/-7 (14 lines); hunks: -38,12 +38,12 @@ jobs:; -82,12 +82,12 @@ jobs:。
- 代码 diff 细节:
...
- 动机: 标题「[feature] Add H20 fp8_w8a8 FusedMoE config for --n-share-experts-fusion=16」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=272,N=128,device_name=NVIDIA_H20,dtype=fp8_w8a8,block_shape=[128, 128].json`；PR 正文摘要: The original fusedMoE config on H20 will become invalid after enabling the new feature --n-share-experts-fusion=16(here tp=16). So we added relevant config to improve the perfor...。
...
- 状态/时间: merged / 2025-04-22
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+9/-3，可读 patch 27 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Turn on DeepGemm By Default and Update Doc」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/deep_gemm.py`, `docs/references/deepseek.md`；PR 正文摘要: Followup of #5580。
- 实现要点: `python/sglang/srt/layers/quantization/deep_gemm.py` modified +1/-1 (2 lines); hunks: -25,7 +25,7；`docs/references/deepseek.md` modified +8/-2 (10 lines); hunks: -138,9 +138,15 @@ With data parallelism attention enabled, we have achieved u...。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-04-23
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+78/-25，可读 patch 209 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fuse q_a_proj and kv_a_proj for DeepSeek models」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: In self attention module of DeepSeek v3, the `q_a_proj` and `kv_a_proj` both takes hidden state as input, so they can be fused into one module, and one launching of DeepGemm is...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +78/-25 (103 lines); hunks: -443,12 +443,12 @@ def __init__(; -470,6 +470,14 @@ def __init__(; symbols: __init__, forward_normal, forward_absorb，涉及 `__init__, forward_normal, forward_absorb`。
- 代码 diff 细节:
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `5d93a950eeeb`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+68/-15，可读 patch 150 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[BugFix] Fix combination of MTP and `--n-share-experts-fusion`with R1」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: We found bugs while using MTP and `-n-share-experts-fusion` together. 1. While using `--n-share-experts-fusion ${TP_SIZE}` with MTP, there will be weight loading error: !image 2...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +18/-14 (32 lines); hunks: -1431,11 +1431,27 @@ def __init__(; -1450,7 +1466,7 @@ def __init__(; symbols: __init__, determine_n_share_experts_fusion, get_input_embeddings，涉及 `__init__, determine_n_share_experts_fusion, get_input_embeddings`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+45/-42，可读 patch 197 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「update triton 3.2.0 h200 fused moe triton config and add warning about triton fused_moe_kernel performance degradation due to different Triton versions.」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=264,N=256,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128].json`, `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`；PR 正文摘要: Reproduction commands :。
- 实现要点: `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=264,N=256,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128].json` modified +41/-41 (82 lines); hunks: -1,102 +1,102; -107,40 +107,40；`python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py` modified +4/-1 (5 lines); hunks: -940,7 +940,10 @@ def get_moe_configs(; symbols: get_moe_configs，涉及 `get_moe_configs`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=264,N=256,device_name=NVIDIA_H200,dtype=fp8_w8a8,block_shape=[128, 128].json`, `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 状态/时间: merged / 2025-04-27
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+27/-27，可读 patch 132 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「perf: update H20 fused_moe_triton kernel config to get higher throughput during prefilling」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=272,N=128,device_name=NVIDIA_H20,dtype=fp8_w8a8,block_shape=[128, 128].json`；PR 正文摘要: We have provided fused_moe_triton config in PR(https://github.com/sgl-project/sglang/pull/5641) to improve model performance. However, we found that after using this config, the...。
- 实现要点: `python/sglang/srt/layers/moe/fused_moe_triton/configs/E=272,N=128,device_name=NVIDIA_H20,dtype=fp8_w8a8,block_shape=[128, 128].json` modified +27/-27 (54 lines); hunks: -5,19 +5,19; -33,25 +33,25。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-04-27
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+100/-9，可读 patch 183 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fuse MLA set kv cache kernel」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/layers/radix_attention.py`, `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Fuse MLA set kv cache kernel and remove k concat operation. Currently only support FA3 backend. Can be applied to other backend with subsequent verification. Accuracy Performanc...。
- 实现要点: `python/sglang/srt/layers/attention/flashattention_backend.py` modified +6/-4 (10 lines); hunks: -625,6 +625,7 @@ def forward_extend(; -639,11 +640,11 @@ def forward_extend(; symbols: forward_extend, forward_decode，涉及 `forward_extend, forward_decode`；`python/sglang/srt/layers/radix_attention.py` modified +5/-2 (7 lines); hunks: -92,8 +92,11 @@ def forward(; symbols: forward，涉及 `forward`；`python/sglang/srt/models/deepseek_v2.py` modified +2/-3 (5 lines); hunks: -757,14 +757,13 @@ def forward_absorb(; symbols: forward_absorb，涉及 `forward_absorb`；`python/sglang/srt/mem_cache/memory_pool.py` modified +87/-0 (87 lines); hunks: -34,6 +34,8; -405,6 +407,72 @@ def copy_two_array(loc, dst_1, src_1, dst_2, src_2, dtype,...; symbols: copy_two_array, set_mla_kv_buffer_kernel, set_mla_kv_buffer_triton, MLATokenToKVPool，涉及 `copy_two_array, set_mla_kv_buffer_kernel, set_mla_kv_buffer_triton`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/attention/flashattention_backend.py`, `python/sglang/srt/layers/radix_attention.py`, `python/sglang/srt/mem_cache/memory_pool.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+305/-3，可读 patch 373 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Add Cutlass MLA attention backend」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/cutlass_mla_backend.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/layers/attention/utils.py`；PR 正文摘要: Enables use of the blackwell cutlass MLA decode kernel with deepseek models. Adds "cutlass_mla" option for attention backend. Deepseek-R1 Benchmarks Using `--attention-backend c...。
- 实现要点: `python/sglang/srt/layers/attention/cutlass_mla_backend.py` added +278/-0 (278 lines); hunks: -0,0 +1,278; symbols: CutlassMLADecodeMetadata, __init__, CutlassMLABackend, init_forward_metadata，涉及 `CutlassMLADecodeMetadata, __init__, CutlassMLABackend`；`python/sglang/srt/model_executor/model_runner.py` modified +7/-0 (7 lines); hunks: -271,6 +271,7 @@ def model_specific_adjustment(self):; -926,6 +927,12 @@ def init_attention_backend(self):; symbols: model_specific_adjustment, init_attention_backend，涉及 `model_specific_adjustment, init_attention_backend`；`python/sglang/srt/layers/attention/utils.py` modified +1/-1 (2 lines); hunks: -49,8 +49,8 @@ def create_flashmla_kv_indices_triton(; symbols: create_flashmla_kv_indices_triton，涉及 `create_flashmla_kv_indices_triton`；`python/sglang/srt/server_args.py` modified +14/-1 (15 lines); hunks: -256,6 +256,12 @@ def __post_init__(self):; -823,7 +829,14 @@ def add_cli_args(parser: argparse.ArgumentParser):; symbols: __post_init__, add_cli_args，涉及 `__post_init__, add_cli_args`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/attention/cutlass_mla_backend.py`, `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/managers/schedule_batch.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+115/-287，可读 patch 522 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Auto set draft model path for MTP」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/model_executor/model_runner.py`；PR 正文摘要: - load nextn layer weights directly from target model - refactor to reuse common weight loading logic for draft model and target model - auto set draft model path if not given W...。
- 实现要点: `python/sglang/srt/models/deepseek_nextn.py` modified +1/-257 (258 lines); hunks: -177,263 +177,7 @@ def forward(; symbols: forward, load_weights，涉及 `forward, load_weights`；`python/sglang/srt/models/deepseek_v2.py` modified +74/-17 (91 lines); hunks: -1502,11 +1502,20 @@ def forward(; -1612,7 +1621,20 @@ def post_load_weights(self):; symbols: forward, post_load_weights, load_weights，涉及 `forward, post_load_weights, load_weights`；`python/sglang/srt/model_executor/model_runner.py` modified +11/-2 (13 lines); hunks: -692,9 +692,14 @@ def profile_max_num_token(self, total_gpu_memory: int):; -809,7 +814,11 @@ def init_memory_pool(; symbols: profile_max_num_token, init_memory_pool，涉及 `profile_max_num_token, init_memory_pool`；`python/sglang/srt/configs/model_config.py` modified +7/-0 (7 lines); hunks: -47,6 +47,7 @@ def __init__(; -85,6 +86,12 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/configs/model_config.py`, `python/sglang/srt/managers/tp_worker.py`, `python/sglang/srt/model_executor/model_runner.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+66/-14，可读 patch 124 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Update ci test and doc for MTP api change」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `test/srt/test_mla_deepseek_v3.py`, `python/sglang/srt/server_args.py`, `docs/references/deepseek.md`；PR 正文摘要: Update ci test and doc for MTP api change.。
- 实现要点: `test/srt/test_mla_deepseek_v3.py` modified +57/-0 (57 lines); hunks: -50,6 +50,63 @@ def test_gsm8k(self):; symbols: test_gsm8k, TestDeepseekV3MTP, setUpClass, tearDownClass，涉及 `test_gsm8k, TestDeepseekV3MTP, setUpClass`；`python/sglang/srt/server_args.py` modified +7/-4 (11 lines); hunks: -347,10 +347,13 @@ def __post_init__(self):; symbols: __post_init__，涉及 `__post_init__`；`docs/references/deepseek.md` modified +2/-4 (6 lines); hunks: -153,12 +153,10 @@ The precompilation process typically takes around 10 minut...；`test/srt/test_full_deepseek_v3.py` modified +0/-2 (2 lines); hunks: -80,8 +80,6 @@ def setUpClass(cls):; symbols: setUpClass，涉及 `setUpClass`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/test_full_deepseek_v3.py`, `test/srt/test_mla_deepseek_v3.py`, `test/srt/test_mla_flashinfer.py`, `test/srt/test_mla_int8_deepseek_v3.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #5908 - feat: Refactor DeepSeekV3 function call

- 链接: https://github.com/sgl-project/sglang/pull/5908
...
- 动机: 标题「feat: Refactor DeepSeekV3 function call」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `examples/chat_template/tool_chat_template_deepseekv3.jinja`；PR 正文摘要: Instead of keeping editing the prompt engineering for `deepseekv3` in `adapter.py`, this PR provides a chat template instead. Implements: https://github.com/sgl-project/sglang/p...。
...
- 状态/时间: merged / 2025-05-02
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+26/-6，可读 patch 95 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Overlap qk norm with two streams」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Overlap qk rms norm with two streams in cuda graph. Accuracy Performance main branch: this PR: Profile main branch: this PR: 23.9μm -> 21.5μm。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +26/-6 (32 lines); hunks: -421,6 +421,7 @@ def __init__(; -543,6 +544,8 @@ def __init__(; symbols: __init__, forward_absorb，涉及 `__init__, forward_absorb`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/6011
- 状态/时间: open / 2025-05-04
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+70/-3，可读 patch 101 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「feat: flashinfer_mla speculative decoding with custom mask」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `test/srt/test_mla_flashinfer.py`, `python/sglang/srt/speculative/eagle_worker.py`；PR 正文摘要: status: not ready. dependent on https://github.com/flashinfer-ai/flashinfer/pull/1017. Support top_k > 1 Eagle speculative decoding with flashinfer MLA backend on custom mask. -...。
- 实现要点: `python/sglang/srt/layers/attention/flashinfer_mla_backend.py` modified +2/-0 (2 lines); hunks: -634,6 +634,7 @@ def call_begin_forward(; -651,6 +652,7 @@ def call_begin_forward(; symbols: call_begin_forward，涉及 `call_begin_forward`；`test/srt/test_mla_flashinfer.py` modified +61/-0 (61 lines); hunks: -162,6 +162,67 @@ def test_gsm8k(self):; symbols: test_gsm8k, TestFlashinferMLAMTPTopK, setUpClass, tearDownClass，涉及 `test_gsm8k, TestFlashinferMLAMTPTopK, setUpClass`；`python/sglang/srt/speculative/eagle_worker.py` modified +7/-3 (10 lines); hunks: -346,9 +346,13 @@ def draft(self, batch: ScheduleBatch):; symbols: draft，涉及 `draft`。
- 代码 diff 细节:
...
- 动机: 标题「Update doc for MLA attention backends」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `docs/references/deepseek.md`, `docs/backend/server_arguments.md`；PR 正文未提供可用摘要。
- 实现要点: `docs/references/deepseek.md` modified +2/-2 (4 lines); hunks: -87,7 +87,7 @@ Please refer to [the example](https://github.com/sgl-project/s...; -158,7 +158,7 @@ Add arguments `--speculative-algorithm`, `--speculative-num-...；`docs/backend/server_arguments.md` modified +1/-1 (2 lines); hunks: -168,7 +168,7 @@ Please consult the documentation below and [server_args.py](...。
- 代码 diff 细节:
...
- 验证与风险: 该 PR 主要落在文档/示例 `docs/backend/server_arguments.md`, `docs/references/deepseek.md`；验证重点是文档命令仍能映射到当前 CLI 参数和模型仓库名。

### PR #6079 - Clean logs for DeepSeek-V3 launching

- 链接: https://github.com/sgl-project/sglang/pull/6079
- 状态/时间: merged / 2025-05-08
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `73600673bb1d`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+61/-39，可读 patch 231 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Clean logs for DeepSeek-V3 launching」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: When launching Deepseek-V3 model, some of the logs only need to be printed once (on tp_rank=0). This PR adds a `should_rank` util function. And print these logs only when `shoul...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +7/-4 (11 lines); hunks: -88,6 +88,7; -1487,8 +1488,9 @@ def determine_n_share_experts_fusion(; symbols: determine_n_share_experts_fusion, get_input_embeddings，涉及 `determine_n_share_experts_fusion, get_input_embeddings`。
- 代码 diff 细节:
...
- 动机: 标题「[perf] dsv3 bmm fallback to bf16」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Align to official impl according to released profile data: https://github.com/deepseek-ai/profile-data/blob/ad8f61ed5e590c4984bb6579f370f9689c427416/decode.json Issue1(fixed): e...。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+444/-87，可读 patch 706 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Feat] Support FlashMLA backend with MTP and FP8 KV cache」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/flashmla_backend.py`, `python/sglang/srt/layers/attention/flashinfer_mla_backend.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`；PR 正文摘要: This PR improves flashmla backend by accelerating decode stage with mtp. The implementation utilizes the feature that flashmla can handle `seq_len_q > 1`. To use flashmla with m...。
- 实现要点: `python/sglang/srt/layers/attention/flashmla_backend.py` modified +340/-78 (418 lines); hunks: -8,7 +8,7; -30,8 +30,8; symbols: __init__, FlashMLABackend, init_forward_metadata，涉及 `__init__, FlashMLABackend, init_forward_metadata`；`python/sglang/srt/layers/attention/flashinfer_mla_backend.py` modified +8/-4 (12 lines); hunks: -346,7 +346,6 @@ def forward_extend(; -381,6 +380,9 @@ def forward_extend(; symbols: forward_extend, forward_decode, __init__，涉及 `forward_extend, forward_decode, __init__`；`python/sglang/srt/model_executor/cuda_graph_runner.py` modified +5/-1 (6 lines); hunks: -30,6 +30,7; -210,7 +211,10 @@ def __init__(self, model_runner: ModelRunner):; symbols: __init__，涉及 `__init__`；`python/sglang/srt/layers/attention/utils.py` modified +2/-2 (4 lines); hunks: -77,8 +77,8 @@ def create_flashmla_kv_indices_triton(; symbols: create_flashmla_kv_indices_triton，涉及 `create_flashmla_kv_indices_triton`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/test_flashmla.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #6151 - [Feat] optimize Qwen3 on H20 by hybrid Attention Backend

- 链接: https://github.com/sgl-project/sglang/pull/6151
- 状态/时间: closed / 2025-05-18
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+39/-9，可读 patch 116 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Feat] optimize Qwen3 on H20 by hybrid Attention Backend」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`, `python/sglang/srt/server_args.py`；PR 正文摘要: FA3 decode performance is significantly lower than flashinfer on H20 5630 https://github.com/Dao-AILab/flash-attention/issues/1572 I have profiled the Qwen3-235B-A22B performanc...。
- 实现要点: `python/sglang/srt/model_executor/model_runner.py` modified +24/-4 (28 lines); hunks: -143,6 +143,7 @@ def __init__(; -977,6 +978,12 @@ def init_attention_backend(self):; symbols: __init__, init_attention_backend, init_double_sparsity_channel_config, apply_torch_tp，涉及 `__init__, init_attention_backend, init_double_sparsity_channel_config`；`python/sglang/srt/model_executor/cuda_graph_runner.py` modified +9/-5 (14 lines); hunks: -211,9 +211,13 @@ def __init__(self, model_runner: ModelRunner):; -445,7 +449,7 @@ def capture_one_batch_size(self, bs: int, forward: Callable):; symbols: __init__, capture_one_batch_size, replay_prepare，涉及 `__init__, capture_one_batch_size, replay_prepare`；`python/sglang/srt/server_args.py` modified +6/-0 (6 lines); hunks: -127,6 +127,7 @@ class ServerArgs:; -873,6 +874,11 @@ def add_cli_args(parser: argparse.ArgumentParser):; symbols: ServerArgs, add_cli_args，涉及 `ServerArgs, add_cli_args`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/model_executor/cuda_graph_runner.py`, `python/sglang/srt/model_executor/model_runner.py`, `python/sglang/srt/server_args.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #6655 - fix(deepseekv3): Fix DeepSeekV3Detector tool_index assignment and multi-tool call streaming support

- 链接: https://github.com/sgl-project/sglang/pull/6655
- 状态/时间: merged / 2025-05-28
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv3_detector.py`；关联提交 `461a73028070`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+52/-4，可读 patch 100 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「fix(deepseekv3): Fix DeepSeekV3Detector tool_index assignment and multi-tool call streaming support」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/function_call/deepseekv3_detector.py`；PR 正文摘要: This PR focuses to fix the current two issues in `DeepSeekV3Detector`. 1. tool_index: Current the tool_index of each ToolCallItem is assigned based on the index of the function...。
- 实现要点: `python/sglang/srt/function_call/deepseekv3_detector.py` modified +52/-4 (56 lines); hunks: -31,6 +31,7 @@ def __init__(self):; -75,7 +76,12 @@ def parse_streaming_increment(; symbols: __init__, has_tool_call, parse_streaming_increment，涉及 `__init__, has_tool_call, parse_streaming_increment`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/function_call/deepseekv3_detector.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #6738 - Partially supports using MHA kernels in MLA forward when page-size > 1.

- 链接: https://github.com/sgl-project/sglang/pull/6738
- 状态/时间: open / 2025-05-29
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+41/-0，可读 patch 83 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Partially supports using MHA kernels in MLA forward when page-size > 1.」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: add Partially support to use MHA kernels when page-size > 1. related to #6411 add AttnForwardMethod.MHA_PAGED_PREFILL, and when any(forward_batch.extend_prefix_lens_cpu) == Fals...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +41/-0 (41 lines); hunks: -152,6 +152,10 @@ class AttnForwardMethod(IntEnum):; -1005,6 +1009,7 @@ def _dispatch_mla_subtype():; symbols: AttnForwardMethod, _dispatch_mla_subtype, forward_prepare, forward_core，涉及 `AttnForwardMethod, _dispatch_mla_subtype, forward_prepare`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/6265
- 状态/时间: merged / 2025-05-29
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `51cdd81f9720`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+47/-14，可读 patch 114 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[fix][RL] Fix DeepSeekV3ForCausalLM.post_load_weights for multiple update weight」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: When doing RL, which will call `post_load_weights` a second time apart from the intialization, the origin `post_load_weights` will somehow corrupt the weight when recreate w_kv...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +39/-14 (53 lines); hunks: -92,6 +92,7; -1713,14 +1714,23 @@ def forward(; symbols: forward, post_load_weights，涉及 `forward, post_load_weights`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/utils.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 动机: 标题「Use deepgemm instead of triton for fused_qkv_a_proj_with_mqa」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8_utils.py`；PR 正文摘要: ~~do not merge now, I will test and check more first~~ Test: * triton: 41us * change to DeepGEMM: 12us thus more than 3x speedup for this kernel change in my scenario。
- 实现要点: `python/sglang/srt/layers/quantization/fp8_utils.py` modified +2/-2 (4 lines); hunks: -227,8 +227,8 @@ def deepgemm_w8a8_block_fp8_linear_with_fallback(; symbols: deepgemm_w8a8_block_fp8_linear_with_fallback，涉及 `deepgemm_w8a8_block_fp8_linear_with_fallback`。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-06-07
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+331/-9，可读 patch 490 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fuse routed scaling factor in topk_reduce kernel」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`；PR 正文摘要: gsm8k acc in h200 benchmark in H200 command: Micro benchmark in h200 micro benchmark in h200: And I use the below code to select triton or `torch.compile` kernel:。
- 实现要点: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py` modified +124/-8 (132 lines); hunks: -1155,6 +1155,7 @@ def inplace_fused_experts(; -1177,6 +1178,8 @@ def inplace_fused_experts(; symbols: inplace_fused_experts, inplace_fused_experts_fake, outplace_fused_experts，涉及 `inplace_fused_experts, inplace_fused_experts_fake, outplace_fused_experts`；`python/sglang/srt/models/deepseek_v2.py` modified +1/-1 (2 lines); hunks: -346,7 +346,7 @@ def forward_normal(self, hidden_states: torch.Tensor) -> tor...; symbols: forward_normal，涉及 `forward_normal`；`python/sglang/srt/layers/moe/fused_moe_triton/layer.py` modified +1/-0 (1 lines); hunks: -225,6 +225,7 @@ def forward_cuda(; symbols: forward_cuda, forward_cpu，涉及 `forward_cuda, forward_cpu`；`python/sglang/srt/layers/quantization/blockwise_int8.py` modified +1/-0 (1 lines); hunks: -411,4 +411,5 @@ def apply(; symbols: apply，涉及 `apply`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/6853
- 状态/时间: merged / 2025-06-08
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c2c4f57f6311`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+386/-13，可读 patch 506 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[DeepseekR1-FP4] Add Support for nvidia/DeepSeekR1-FP4 model」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Adds Model support for DeepSeek R1 FP4 Model. (Functional enablement - kernel optimizations underway) Adds ModelOptFP4FusedMoEMethod and CutlassMoEParams dataclass to initialize...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +33/-5 (38 lines); hunks: -1746,7 +1746,7 @@ def determine_num_fused_shared_experts(; -1926,6 +1926,7 @@ def post_load_weights(self, is_nextn=False, weight_names=N...; symbols: determine_num_fused_shared_experts, get_input_embeddings, post_load_weights, load_weights，涉及 `determine_num_fused_shared_experts, get_input_embeddings, post_load_weights`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/layers/quantization/modelopt_quant.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 状态/时间: merged / 2025-06-08
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+338/-15，可读 patch 498 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fuse routed scaling factor in deepseek」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py` modified +130/-14 (144 lines); hunks: -1155,6 +1155,7 @@ def inplace_fused_experts(; -1177,6 +1178,8 @@ def inplace_fused_experts(; symbols: inplace_fused_experts, inplace_fused_experts_fake, outplace_fused_experts，涉及 `inplace_fused_experts, inplace_fused_experts_fake, outplace_fused_experts`；`python/sglang/srt/models/deepseek_v2.py` modified +2/-1 (3 lines); hunks: -346,7 +346,8 @@ def forward_normal(self, hidden_states: torch.Tensor) -> tor...; symbols: forward_normal，涉及 `forward_normal`；`python/sglang/srt/layers/moe/fused_moe_triton/layer.py` modified +1/-0 (1 lines); hunks: -225,6 +225,7 @@ def forward_cuda(; symbols: forward_cuda, forward_cpu，涉及 `forward_cuda, forward_cpu`；`python/sglang/srt/layers/quantization/blockwise_int8.py` modified +1/-0 (1 lines); hunks: -411,4 +411,5 @@ def apply(; symbols: apply，涉及 `apply`。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+92/-44，可读 patch 216 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support new DeepGEMM format in per token group quant」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `sgl-kernel/csrc/gemm/per_token_group_quant_8bit.cu`, `sgl-kernel/tests/test_per_token_group_quant_8bit.py`, `sgl-kernel/include/sgl_kernel_ops.h`；PR 正文未提供可用摘要。
- 实现要点: `sgl-kernel/csrc/gemm/per_token_group_quant_8bit.cu` modified +83/-40 (123 lines); hunks: -16,11 +16,16 @@ __device__ __forceinline__ float GroupReduceMax(float val, c...; -39,15 +44,24 @@ __global__ void per_token_group_quant_8bit_kernel(；`sgl-kernel/tests/test_per_token_group_quant_8bit.py` modified +4/-1 (5 lines); hunks: -255,7 +255,10 @@ def sglang_per_token_group_quant_8bit(; symbols: sglang_per_token_group_quant_8bit，涉及 `sglang_per_token_group_quant_8bit`；`sgl-kernel/include/sgl_kernel_ops.h` modified +2/-1 (3 lines); hunks: -175,7 +175,8 @@ void sgl_per_token_group_quant_fp8(；`sgl-kernel/python/sgl_kernel/gemm.py` modified +2/-1 (3 lines); hunks: -90,9 +90,10 @@ def sgl_per_token_group_quant_fp8(; symbols: sgl_per_token_group_quant_fp8，涉及 `sgl_per_token_group_quant_fp8`。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-06-13
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+19/-4，可读 patch 59 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support new DeepGEMM format in per token group quant (part 2: srt)」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/entrypoints/engine.py`, `python/pyproject.toml`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/layers/quantization/fp8_kernel.py` modified +17/-2 (19 lines); hunks: -280,15 +280,28 @@ def sglang_per_token_group_quant_fp8(; -309,7 +322,9 @@ def sglang_per_token_group_quant_fp8(; symbols: sglang_per_token_group_quant_fp8，涉及 `sglang_per_token_group_quant_fp8`；`python/sglang/srt/entrypoints/engine.py` modified +1/-1 (2 lines); hunks: -605,7 +605,7 @@ def _set_envs_and_config(server_args: ServerArgs):; symbols: _set_envs_and_config，涉及 `_set_envs_and_config`；`python/pyproject.toml` modified +1/-1 (2 lines); hunks: -49,7 +49,7 @@ runtime_common = [。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/pyproject.toml`, `python/sglang/srt/entrypoints/engine.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 状态/时间: merged / 2025-06-13
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+125/-0，可读 patch 154 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Re-quantize DeepSeek model weights to support DeepGEMM new input format」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8_utils.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/math_utils.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/layers/quantization/fp8_utils.py` modified +61/-0 (61 lines); hunks: -4,6 +4,7; -390,6 +391,66 @@ def block_quant_dequant(; symbols: block_quant_dequant, requant_weight_ue8m0_inplace, _requant_weight_ue8m0, _transform_scale，涉及 `block_quant_dequant, requant_weight_ue8m0_inplace, _requant_weight_ue8m0`；`python/sglang/srt/models/deepseek_v2.py` modified +56/-0 (56 lines); hunks: -66,6 +66,7; -1931,6 +1932,61 @@ def post_load_weights(self, is_nextn=False, weight_names=...; symbols: post_load_weights, _weight_requant_ue8m0, load_weights，涉及 `post_load_weights, _weight_requant_ue8m0, load_weights`；`python/sglang/math_utils.py` added +8/-0 (8 lines); hunks: -0,0 +1,8; symbols: align, ceil_div，涉及 `align, ceil_div`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/math_utils.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 12 个文件，+207/-147，可读 patch 685 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Refactor DeepGEMM integration」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/deep_gemm_wrapper/compile_utils.py`, `python/sglang/srt/layers/quantization/deep_gemm_wrapper/entrypoint.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/layers/quantization/deep_gemm_wrapper/compile_utils.py` renamed +22/-76 (98 lines); hunks: -5,33 +5,24; -52,8 +43,10 @@ def get_enable_jit_deepgemm():; symbols: get_enable_jit_deepgemm, DeepGemmKernelHelper, _compile_warning_1，涉及 `get_enable_jit_deepgemm, DeepGemmKernelHelper, _compile_warning_1`；`python/sglang/srt/layers/quantization/deep_gemm_wrapper/entrypoint.py` added +95/-0 (95 lines); hunks: -0,0 +1,95; symbols: grouped_gemm_nt_f8f8bf16_masked, grouped_gemm_nt_f8f8bf16_contig, gemm_nt_f8f8bf16, update_deep_gemm_config，涉及 `grouped_gemm_nt_f8f8bf16_masked, grouped_gemm_nt_f8f8bf16_contig, gemm_nt_f8f8bf16`；`python/sglang/srt/layers/moe/ep_moe/layer.py` modified +35/-32 (67 lines); hunks: -1,30 +1,11; -45,17 +26,28; symbols: create_weights, __init__, forward, forward_deepgemm_contiguous，涉及 `create_weights, __init__, forward`；`python/sglang/srt/layers/quantization/deep_gemm_wrapper/configurer.py` added +26/-0 (26 lines); hunks: -0,0 +1,26; symbols: _compute_enable_deep_gemm，涉及 `_compute_enable_deep_gemm`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/layers/moe/ep_moe/token_dispatcher.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 状态/时间: merged / 2025-06-14
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+59/-19，可读 patch 161 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support new DeepGEMM」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/deep_gemm_wrapper/entrypoint.py`, `python/sglang/srt/layers/quantization/fp8_kernel.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/layers/quantization/deep_gemm_wrapper/entrypoint.py` modified +18/-8 (26 lines); hunks: -16,14 +16,24; symbols: grouped_gemm_nt_f8f8bf16_masked，涉及 `grouped_gemm_nt_f8f8bf16_masked`；`python/sglang/srt/layers/quantization/fp8_kernel.py` modified +20/-3 (23 lines); hunks: -765,16 +765,33 @@ def prepare_block_fp8_matmul_inputs(; symbols: prepare_block_fp8_matmul_inputs，涉及 `prepare_block_fp8_matmul_inputs`；`python/sglang/srt/layers/moe/ep_moe/layer.py` modified +8/-1 (9 lines); hunks: -1231,14 +1231,21 @@ def forward_deepgemm_masked(; symbols: forward_deepgemm_masked，涉及 `forward_deepgemm_masked`；`python/sglang/srt/layers/quantization/deep_gemm_wrapper/configurer.py` modified +7/-1 (8 lines); hunks: -21,6 +21,12 @@ def _compute_enable_deep_gemm():; symbols: _compute_enable_deep_gemm，涉及 `_compute_enable_deep_gemm`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/moe/ep_moe/layer.py`, `python/sglang/srt/layers/moe/ep_moe/token_dispatcher.py`, `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #7181 - [fix] fix dsv3 weight loader tqdm and simplify shared experts fusion

- 链接: https://github.com/sgl-project/sglang/pull/7181
- 状态/时间: merged / 2025-06-14
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `ed54bf9d19f1`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+11/-96，可读 patch 131 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[fix] fix dsv3 weight loader tqdm and simplify shared experts fusion」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Using `weights = [w for w in weights_list if w[0] not in names_to_remove]` to update `weights` will change it from async weight iterator to a list. Thats will make the tqdm look...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +11/-96 (107 lines); hunks: -1033,7 +1033,7 @@ def forward_absorb_core(; -2008,101 +2008,6 @@ def load_weights(self, weights: Iterable[Tuple[str, torc...; symbols: forward_absorb_core, load_weights，涉及 `forward_absorb_core, load_weights`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7180
- 状态/时间: merged / 2025-06-15
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+29/-47，可读 patch 83 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[fix] fix determine_num_fused_shared_experts」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +29/-47 (76 lines); hunks: -1709,53 +1709,35 @@ def routed_experts_weights_of_layer(self):; symbols: routed_experts_weights_of_layer, determine_num_fused_shared_experts, get_input_embeddings，涉及 `routed_experts_weights_of_layer, determine_num_fused_shared_experts, get_input_embeddings`。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 22 个文件，+636/-146，可读 patch 1516 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「feat: mtp support dp-attention」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/model_executor/cuda_graph_runner.py`, `python/sglang/srt/layers/attention/triton_backend.py`；PR 正文摘要: mtp support dp-attention - implemented MTP for DP-attention, also fixed related bugs #4783 #4847 . - Enabled CUDA Graph support for both target and draft models at dp-attention....。
- 实现要点: `python/sglang/srt/models/deepseek_nextn.py` modified +18/-26 (44 lines); hunks: -22,7 +22,6; -77,6 +76,7 @@ def forward(; symbols: forward, __init__，涉及 `forward, __init__`；`python/sglang/srt/model_executor/cuda_graph_runner.py` modified +21/-15 (36 lines); hunks: -242,13 +242,13 @@ def __init__(self, model_runner: ModelRunner):; -323,12 +323,15 @@ def __init__(self, model_runner: ModelRunner):; symbols: __init__, can_run, capture_one_batch_size, replay_prepare，涉及 `__init__, can_run, capture_one_batch_size`；`python/sglang/srt/layers/attention/triton_backend.py` modified +19/-11 (30 lines); hunks: -261,6 +261,7 @@ def init_forward_metadata(self, forward_batch: ForwardBatch):; -335,24 +336,27 @@ def init_forward_metadata(self, forward_batch: ForwardBatch):; symbols: init_forward_metadata, init_cuda_graph_state, init_forward_metadata_capture_cuda_graph，涉及 `init_forward_metadata, init_cuda_graph_state, init_forward_metadata_capture_cuda_graph`；`python/sglang/srt/model_executor/forward_batch_info.py` modified +18/-5 (23 lines); hunks: -320,17 +320,30 @@ def init_new(; symbols: init_new，涉及 `init_new`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/test_dp_attention.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #7164 - Fix Deepseek R1 0528 FP4 tensor name mismatch issue during weights loading.

- 链接: https://github.com/sgl-project/sglang/pull/7164
- 状态/时间: merged / 2025-06-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `8c16da334e93`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+10/-7，可读 patch 45 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix Deepseek R1 0528 FP4 tensor name mismatch issue during weights loading.」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This PR tries to fix a weights loading issue for Deepseek R1 0528 Fp4 from Nvidia. To reproduce the issue: (i'm using blackwell image) If we look into the weight file and model...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +5/-6 (11 lines); hunks: -1919,6 +1919,8 @@ def post_load_weights(self, is_nextn=False, weight_names=N...; -2151,12 +2153,9 @@ def load_weights(self, weights: Iterable[Tuple[str, torch...; symbols: post_load_weights, load_weights，涉及 `post_load_weights, load_weights`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/model_loader/weight_utils.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #7286 - fix: resolve b200 dsv3 mtp issue

- 链接: https://github.com/sgl-project/sglang/pull/7286
- 状态/时间: merged / 2025-06-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `4f204db57cf9`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-0，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「fix: resolve b200 dsv3 mtp issue」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +2/-0 (2 lines); hunks: -1932,6 +1932,8 @@ def post_load_weights(self, is_nextn=False, weight_names=N...; symbols: post_load_weights, _weight_requant_ue8m0，涉及 `post_load_weights, _weight_requant_ue8m0`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7371
- 状态/时间: merged / 2025-06-20
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `a06912ad8b35`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+3/-3，可读 patch 24 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix judgment condition for enabling Deepseek V3/R1 shared expert fusion optimization」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: ref: https://github.com/sgl-project/sglang/pull/7180 Modify the compatibility version judgment conditions. The following results were tested in the A800 environment: - main - pr。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +3/-3 (6 lines); hunks: -1731,12 +1731,12 @@ def determine_num_fused_shared_experts(; -2040,7 +2040,7 @@ def load_weights(self, weights: Iterable[Tuple[str, torch....; symbols: determine_num_fused_shared_experts, load_weights，涉及 `determine_num_fused_shared_experts, load_weights`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7481
- 状态/时间: merged / 2025-06-23
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `bdbb8d009ae1`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+4/-2，可读 patch 17 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[perf] slightly imporve DeepSeek-R1-FP4 TP8」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Tuned dual stream kernel order, 79->81。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +4/-2 (6 lines); hunks: -362,12 +362,14 @@ def forward(; symbols: forward, forward_normal_dual_stream，涉及 `forward, forward_normal_dual_stream`。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-06-24
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `15f34013432f`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+20/-1，可读 patch 49 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix MTP with Deepseek R1 Fp4」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: https://github.com/sgl-project/sglang/issues/7365 Update quant config to None for MTP module in Deepseek modelopt fp4. It's not quantized. See https://github.com/NVIDIA/TensorRT...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +8/-1 (9 lines); hunks: -2201,7 +2201,7 @@ def load_weights(self, weights: Iterable[Tuple[str, torch....; -2232,6 +2232,13 @@ def load_weights(self, weights: Iterable[Tuple[str, torch...; symbols: load_weights，涉及 `load_weights`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #7635 - Apply dsv3_fused_a_gemm kernel

- 链接: https://github.com/sgl-project/sglang/pull/7635
- 状态/时间: merged / 2025-07-02
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `00aec6ad6c34`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+18/-2，可读 patch 48 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Apply dsv3_fused_a_gemm kernel」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Apply `dsv3_fused_a_gemm kernel` in https://github.com/sgl-project/sglang/pull/7630. Profile and accuracy test are already listed in the previous PR. Dependency: release new ver...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +18/-2 (20 lines); hunks: -96,6 +96,7; -112,7 +113,7; symbols: __init__, forward_absorb_prepare，涉及 `__init__, forward_absorb_prepare`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7677
- 状态/时间: merged / 2025-07-02
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `88f484ce4c73`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+21/-3，可读 patch 47 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Apply dsv3 router gemm kernel for deepseek-r1 fp4」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Apply kernel added in #7627 to dpsk-r1 fp4 model. Currently we have to convert the dtype of router logits back to bf16, otherwise there will be cuda illegal memory bug. We shoul...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +21/-3 (24 lines); hunks: -111,9 +111,16; -225,7 +232,18 @@ def forward(self, hidden_states):; symbols: forward, __init__，涉及 `forward, __init__`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7735
- 状态/时间: merged / 2025-07-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `84f2e4a0f847`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+15/-2，可读 patch 47 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「fix awq and dsv3 fused gemm compatible」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: added a check to avoid compatibility issues when loading AWQ weights with dsv3 fused gemm enabled related issue https://github.com/sgl-project/sglang/issues/7728。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +15/-2 (17 lines); hunks: -318,11 +318,17 @@ def __init__(; -876,8 +882,13 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7738
- 状态/时间: merged / 2025-07-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `b58226510f7d`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+15/-14，可读 patch 61 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「fix dsv3 fused proj check」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: fix dsv3 fused proj check in https://github.com/sgl-project/sglang/pull/7735。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +15/-14 (29 lines); hunks: -336,10 +336,12 @@ def __init__(; -891,21 +893,20 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7750
- 状态/时间: merged / 2025-07-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `ac49dac00946`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+2/-1，可读 patch 10 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[fix] fix dsv3_router_gemm filter」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: fix ci on main: https://github.com/sgl-project/sglang/actions/runs/16045971205/job/45277302233#step:5:3827。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +2/-1 (3 lines); hunks: -233,7 +233,8 @@ def forward(self, hidden_states):; symbols: forward，涉及 `forward`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7793
- 状态/时间: merged / 2025-07-06
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `54411f6afa27`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+9/-1，可读 patch 49 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「fix: disable dsv3_router_gemm in dsv3_nextn」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: It has been confirmed that using dsv3_router_gemm leads to a degradation in the acc_len of the dsr1-fp4 nextn module.: https://github.com/sgl-project/sglang/pull/7711/files#diff...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +9/-1 (10 lines); hunks: -210,8 +210,10 @@ def __init__(; -233,8 +235,10 @@ def forward(self, hidden_states):; symbols: __init__, forward，涉及 `__init__, forward`。
- 代码 diff 细节:
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `cb9d91ea8a71`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+1006/-9，可读 patch 1203 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「feat: support DeepSeek-R1-W4AFP8 model with ep-moe mode」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This PR supports running DeepSeek-R1-W4AFP8 model with ep-moe mode(deepep mode support is on the way~) Due to the reduced space required for model weights and decreased bandwidt...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +6/-0 (6 lines); hunks: -2363,6 +2363,12 @@ def load_weights(self, weights: Iterable[Tuple[str, torch...; symbols: load_weights，涉及 `load_weights`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/7562
- 状态/时间: merged / 2025-07-24
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/function_call/deepseekv3_detector.py`；关联提交 `624a3b8d1f10`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+90/-1，可读 patch 104 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix incomplete tool call capture issue in streaming response of DeepSeek-V3 when enable MTP」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/function_call/deepseekv3_detector.py`；PR 正文摘要: When using DeepSeeker V3 and enabling MTP and tool call parsing, we found that there are have some unexpected string in the streaming reponse like " \n ". The problem is the reg...。
- 实现要点: `python/sglang/srt/function_call/deepseekv3_detector.py` modified +1/-1 (2 lines); hunks: -98,7 +98,7 @@ def parse_streaming_increment(; symbols: parse_streaming_increment，涉及 `parse_streaming_increment`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/8311
- 状态/时间: closed / 2025-07-28
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 219 个文件，+16292/-3879，可读 patch 27289 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support DeepSeek-R1 w4a8 low latency deepep」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/glm4_moe.py`, `test/srt/test_function_call_parser.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`；PR 正文摘要: Follow https://github.com/sgl-project/sglang/pull/8247 https://github.com/sgl-project/sglang/pull/7762. Support deepep low latency mode for DeepSeek-R1 w4a8 model add forward_cu...。
- 实现要点: `python/sglang/srt/models/glm4_moe.py` added +1035/-0 (1035 lines); hunks: -0,0 +1,1035; symbols: Glm4MoeMLP, __init__, forward, Glm4MoeAttention，涉及 `Glm4MoeMLP, __init__, forward`；`test/srt/test_function_call_parser.py` modified +639/-0 (639 lines); hunks: -6,10 +6,12; -507,7 +509,9 @@ def setUp(self):; symbols: setUp, test_pythonic_detector_ebnf, test_qwen25_detector_ebnf, test_glm45_detector_ebnf，涉及 `setUp, test_pythonic_detector_ebnf, test_qwen25_detector_ebnf`；`python/sglang/srt/layers/moe/ep_moe/layer.py` modified +211/-203 (414 lines); hunks: -47,13 +47,15; -64,6 +66,13; symbols: forward, EPMoE, _get_tile_tokens_dim, __init__，涉及 `forward, EPMoE, _get_tile_tokens_dim`；`test/srt/openai_server/function_call/test_openai_function_calling.py` modified +360/-4 (364 lines); hunks: -16,6 +16,20; -73,9 +87,13 @@ def test_function_calling_format(self):; symbols: TestOpenAIServerFunctionCalling, setUpClass, test_function_calling_format, _test_function_calling_multiturn，涉及 `TestOpenAIServerFunctionCalling, setUpClass, test_function_calling_format`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `python/sglang/test/runners.py`, `python/sglang/test/test_activation.py`, `python/sglang/test/test_utils.py`, `sgl-kernel/tests/test_kvcacheio.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #8051 - Update batch size limitation of dsv3_router_gemm kernel to 16

- 链接: https://github.com/sgl-project/sglang/pull/8051
- 状态/时间: merged / 2025-08-01
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `e7e5a3050a64`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-2，可读 patch 10 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Update batch size limitation of dsv3_router_gemm kernel to 16」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Raise the batch size limitation to from 4 to 16 for dsv3_router_gemm kernel。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +1/-2 (3 lines); hunks: -248,8 +248,7 @@ def forward(self, hidden_states):; symbols: forward，涉及 `forward`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/9525
- 状态/时间: merged / 2025-08-23
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv3.jinja`, `python/sglang/srt/function_call/deepseekv3_detector.py`；关联提交 `c9dd70fbde92`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+17/-17，可读 patch 98 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「tool-call(dsv3): Improve deepseek-v3 chat template and tool_choice = `required`」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `python/sglang/srt/function_call/deepseekv3_detector.py`, `examples/chat_template/tool_chat_template_deepseekv3.jinja`；PR 正文摘要: Improve deepseek-v3 chat template, following #8576 - Improve deepseek-v3 chat template - Remove spaces in the `build_ebnf` Actions Please wait for CI to pass, to make sure this...。
- 实现要点: `python/sglang/srt/function_call/deepseekv3_detector.py` modified +1/-1 (2 lines); hunks: -215,6 +215,6 @@ def build_ebnf(self, tools: List[Tool]):; symbols: build_ebnf，涉及 `build_ebnf`；`examples/chat_template/tool_chat_template_deepseekv3.jinja` modified +16/-16 (32 lines); hunks: -12,7 +12,7; -23,13 +23,13。
- 代码 diff 细节:
...
- 动机: 标题「Support DeepSeek-V3.1 tool call」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/function_call/deepseekv31_detector.py`, `examples/chat_template/tool_chat_template_deepseekv31.jinja`；PR 正文摘要: Support tool call for DeepSeek-V3.1 The tool call format of DeepSeek-V3.1 is different from DeepSeek-V3/R1: DeepSeek-V3.1: tool_call_name tool_call_arguments DeepSeek-R1/V3: fun...。
...
- 链接: https://github.com/sgl-project/sglang/pull/9815
- 状态/时间: merged / 2025-08-30
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `8abe8deae6cd`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+12/-4，可读 patch 31 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「fix: dsv3 lite q_lora_rank none」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +12/-4 (16 lines); hunks: -2414,18 +2414,26 @@ def _weight_requant_ue8m0(self, is_nextn=False):; symbols: _weight_requant_ue8m0，涉及 `_weight_requant_ue8m0`。
- 代码 diff 细节:
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `d4a938417d2c`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 11 个文件，+291/-120，可读 patch 710 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[feat] Support tp mode for DeepSeek-R1-W4AFP8」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Support tp mode for DeepSeek w4a8 model, which has a better performace than ep mode. 1. Add W4AFp8MoEMethod and associated `create_weights`, `process_weights_after_loading` func...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +5/-0 (5 lines); hunks: -2168,6 +2168,8 @@ def determine_num_fused_shared_experts(; -2471,6 +2473,9 @@ def load_weights(self, weights: Iterable[Tuple[str, torch....; symbols: determine_num_fused_shared_experts, load_weights，涉及 `determine_num_fused_shared_experts, load_weights`。
- 代码 diff 细节:
...
- 动机: 标题「Update tool_chat_template_deepseekv31.jinja」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `examples/chat_template/tool_chat_template_deepseekv31.jinja`；PR 正文摘要: 增加tojson，解决多轮工具调用报错的问题： https://github.com/sgl-project/sglang/issues/9891。
...
- 状态/时间: merged / 2025-09-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `0dfd54d11d06`, `1b2ff4fb7f05`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+458/-62，可读 patch 763 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Optimized deepseek-v3/r1 model performance on mxfp4 run」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: In order to decrease the activation tensor quantized overhead, fused the quantized behavior into the different ops (activation, layernorm, gemm, flatten) Test below commands, we...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +202/-27 (229 lines); hunks: -112,6 +112,7; -129,6 +130,22; symbols: forward, __init__，涉及 `forward, __init__`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/9959
- 状态/时间: merged / 2025-09-03
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `1b2ff4fb7f05`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+62/-458，可读 patch 763 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Revert "Optimized deepseek-v3/r1 model performance on mxfp4 run (#9671)"」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This reverts commit 0dfd54d11d06eb8363bc7fb3cf9a1f464368caf8. Because #9671 breaks on B200。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +27/-202 (229 lines); hunks: -112,7 +112,6; -130,22 +129,6; symbols: forward, __init__，涉及 `forward, __init__`。
- 代码 diff 细节:
...
- 状态/时间: merged / 2025-09-04
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `918e3d4c27c3`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+100/-69，可读 patch 210 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix accuracy drop of dsv3 run in dp enablement」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Got an accuracy issue of dsv3 model run on aiter backend with dp enablement. Issue link: https://github.com/sgl-project/sglang/issues/7692 For extend attention, use absorb atten...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +7/-1 (8 lines); hunks: -1077,7 +1077,13 @@ def _dispatch_mla_subtype():; symbols: _dispatch_mla_subtype，涉及 `_dispatch_mla_subtype`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/9834
- 状态/时间: merged / 2025-09-06
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `012584ecd539`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+13/-4，可读 patch 52 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「perf: Avoid unnecessary data type conversions for DeepSeek-V3 on Blackwell」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Optimize the performance of DeepSeek-V3 on Blackwell for small batch sizes by - Change the output data type of router GEMM to FP32 - Convert the data type of `correction_bias` i...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +11/-3 (14 lines); hunks: -67,7 +67,10; -299,7 +302,9 @@ def forward(self, hidden_states, gemm_output_zero_allocator:...; symbols: forward, __init__，涉及 `forward, __init__`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/entrypoints/engine.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #10104 - Fix run time error in dsv3-fp8 model on mi35x

- 链接: https://github.com/sgl-project/sglang/pull/10104
- 状态/时间: merged / 2025-09-08
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `400d3b97aebc`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-1，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix run time error in dsv3-fp8 model on mi35x」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Got run-time error in running dsv3-fp8 model on mi35x. The error is caused by the activation tensor is changed to tuple of tensors type for the next mxfp4 linear computation. Bu...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +5/-1 (6 lines); hunks: -249,7 +249,11 @@ def forward(; symbols: forward，涉及 `forward`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/10209
- 状态/时间: merged / 2025-09-09
- 反查来源: `git log --name-only -- <model-files>` 反查到 `examples/chat_template/tool_chat_template_deepseekv3.jinja`；关联提交 `2cd94dd07eea`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「tool-call(dsv3): Fixed a parse problem when there are multiple function definitions in tool_calls」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `examples/chat_template/tool_chat_template_deepseekv3.jinja`；PR 正文摘要: When there are multiple function definitions in tool_calls, an error will be reported **TypeError: can only concatenate str (not "dict") to str** Add what is missing `|tojson` r...。
- 实现要点: `examples/chat_template/tool_chat_template_deepseekv3.jinja` modified +1/-1 (2 lines); hunks: -55,7 +55,7。
- 代码 diff 细节:
...
- 验证与风险: 该 PR 主要落在文档/示例 `examples/chat_template/tool_chat_template_deepseekv3.jinja`；验证重点是文档命令仍能映射到当前 CLI 参数和模型仓库名。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+39/-5，可读 patch 85 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix GPU fault issue when run dsv3 with dp mode and enable torch-compile」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/dp_attention.py`, `python/sglang/srt/layers/logits_processor.py`；PR 正文摘要: Got GPU fault when running "python3 -m sglang.launch_server --model-path DeepSeek-V3.1 --tp 8 --trust-remote-code --chunked-prefill-size 131072 --dp-size 8 --enable-dp-attention...。
- 实现要点: `python/sglang/srt/layers/dp_attention.py` modified +24/-0 (24 lines); hunks: -119,6 +119,18 @@ def get_local_dp_buffer_len(cls) -> int:; -150,6 +162,18 @@ def get_dp_global_num_tokens() -> List[int]:; symbols: get_local_dp_buffer_len, get_dp_global_num_tokens, get_dp_hidden_size, get_dp_dtype，涉及 `get_local_dp_buffer_len, get_dp_global_num_tokens, get_dp_hidden_size`；`python/sglang/srt/layers/logits_processor.py` modified +15/-5 (20 lines); hunks: -35,6 +35,9; -187,16 +190,23 @@ def compute_dp_attention_metadata(self):; symbols: compute_dp_attention_metadata, _get_logits，涉及 `compute_dp_attention_metadata, _get_logits`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/dp_attention.py`, `python/sglang/srt/layers/logits_processor.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 动机: 标题「Add DeepSeek-V3.2 Tool Call Template」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `examples/chat_template/tool_chat_template_deepseekv32.jinja`；PR 正文未提供可用摘要。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+26/-1，可读 patch 34 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Update DeepSeek-R1-FP4 default config on blackwell」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/server_args.py`；PR 正文未提供可用摘要。
- 实现要点: `python/sglang/srt/server_args.py` modified +26/-1 (27 lines); hunks: -799,7 +799,32 @@ def _handle_model_specific_adjustments(self):; symbols: _handle_model_specific_adjustments，涉及 `_handle_model_specific_adjustments`。
- 代码 diff 细节:
...
- 动机: 标题「[DSv32] Use torch.compile for _get_logits_head_gate」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；PR 正文摘要: fuse 2 unary mul + 1 binary mul in _get_logits_head_gate. 1.8% e2e total throughput With torch.compile Without。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+334/-7，可读 patch 475 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[1/N]Support DeepSeek-R1 w4a8 normal deepep」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`；PR 正文摘要: Support deepep normal mode for DeepSeek w4a8 model with @yangsijia-serena @rainj-me add forward_cutlass_w4a8 for deepep normal mode Command `SGL_ENABLE_JIT_DEEPGEMM=1 python3 -m...。
- 实现要点: `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py` modified +196/-0 (196 lines); hunks: -1,5 +1,6; -11,6 +12,9; symbols: cutlass_w4a8_moe, cutlass_w4a8_moe_deepep_normal，涉及 `cutlass_w4a8_moe, cutlass_w4a8_moe_deepep_normal`；`python/sglang/srt/layers/quantization/w4afp8.py` modified +47/-1 (48 lines); hunks: -1,7 +1,7; -21,8 +21,10; symbols: apply, apply_deepep_normal，涉及 `apply, apply_deepep_normal`；`python/sglang/srt/layers/moe/ep_moe/layer.py` modified +21/-2 (23 lines); hunks: -29,6 +29,7; -95,6 +96,11 @@ def __init__(; symbols: __init__, moe_impl, forward_flashinfer_cutedsl, forward_cutlass_w4afp8，涉及 `__init__, moe_impl, forward_flashinfer_cutedsl`；`python/sglang/srt/layers/moe/token_dispatcher/deepep.py` modified +10/-4 (14 lines); hunks: -14,7 +14,12; -340,7 +345,10 @@ def dispatch_a(; symbols: dispatch_a, _dispatch_core，涉及 `dispatch_a, _dispatch_core`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/quant/test_w4a8_deepseek_v3.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 动机: 标题「[Bugfix] Fix Qwen3/DSV3/DSV3.2 model support」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This pr fixes very models that failed to start on npu after dsv3.2 support pr was merged. ci and image release infra was also improved. - Fix mla prefill padding mismatch with a...。
...
- 动机: 标题「[Draft] Support MTP for DeepSeek-V3.2」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/configs/model_config.py`；PR 正文未提供可用摘要。
...
- 动机: 标题「Cleaning indexer for DeepSeek V3.2」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa/utils.py`；PR 正文摘要: - Fake indexer is for testing accuracy during early development, so can be deprecated now. - Cleaning forward logic.。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+64/-5，可读 patch 132 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[1/2] deepseek deterministic: support deterministic inference for deepseek arch models on a single GPU」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/batch_invariant_ops/batch_invariant_ops.py`, `python/sglang/srt/server_args.py`；PR 正文摘要: Part of this Issue: https://github.com/sgl-project/sglang/issues/10278 As part of deepseek deterministic inference support, this change ensures deterministic inference results f...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +9/-1 (10 lines); hunks: -350,7 +350,11 @@ def handle_attention_flashinfer(attn, forward_batch):; -394,6 +398,10 @@ def handle_attention_nsa(attn, forward_batch):; symbols: handle_attention_flashinfer, handle_attention_fa3, handle_attention_flashmla, handle_attention_nsa，涉及 `handle_attention_flashinfer, handle_attention_fa3, handle_attention_flashmla`；`python/sglang/srt/batch_invariant_ops/batch_invariant_ops.py` modified +31/-2 (33 lines); hunks: -495,16 +495,37 @@ def mean_batch_invariant(input, dim, keepdim=False, dtype:...; -516,12 +537,20 @@ def enable_batch_invariant_mode():; symbols: mean_batch_invariant, bmm_batch_invariant, is_batch_invariant_mode_enabled, enable_batch_invariant_mode，涉及 `mean_batch_invariant, bmm_batch_invariant, is_batch_invariant_mode_enabled`；`python/sglang/srt/server_args.py` modified +24/-2 (26 lines); hunks: -1532,13 +1532,30 @@ def _handle_deterministic_inference(self):; -1553,8 +1570,13 @@ def _handle_deterministic_inference(self):; symbols: _handle_deterministic_inference，涉及 `_handle_deterministic_inference`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/batch_invariant_ops/batch_invariant_ops.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/server_args.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 8 个文件，+531/-9，可读 patch 648 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[2/N]Support DeepSeek-R1 w4a8 low latency deepep」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`；PR 正文摘要: Follow https://github.com/sgl-project/sglang/pull/8247 https://github.com/sgl-project/sglang/pull/7762. Based on https://github.com/sgl-project/sglang/pull/8311. Support deepep...。
- 实现要点: `python/sglang/srt/layers/moe/ep_moe/kernels.py` modified +194/-0 (194 lines); hunks: -1014,3 +1014,197 @@ def zero_experts_compute_triton(; symbols: zero_experts_compute_triton, compute_problem_sizes_w4a8_kernel, compute_problem_sizes_w4a8, deepep_ll_get_cutlass_w4a8_moe_mm_data，涉及 `zero_experts_compute_triton, compute_problem_sizes_w4a8_kernel, compute_problem_sizes_w4a8`；`python/sglang/srt/layers/moe/cutlass_w4a8_moe.py` modified +138/-0 (138 lines); hunks: -11,12 +11,14; -396,3 +398,139 @@ def cutlass_w4a8_moe_deepep_normal(; symbols: cutlass_w4a8_moe_deepep_normal, cutlass_w4a8_moe_deepep_ll，涉及 `cutlass_w4a8_moe_deepep_normal, cutlass_w4a8_moe_deepep_ll`；`python/sglang/srt/layers/quantization/w4afp8.py` modified +36/-0 (36 lines); hunks: -23,6 +23,7; -328,6 +329,41 @@ def apply(; symbols: apply, apply_deepep_ll, apply_deepep_normal，涉及 `apply, apply_deepep_ll, apply_deepep_normal`；`python/sglang/srt/layers/moe/ep_moe/layer.py` modified +17/-0 (17 lines); hunks: -100,6 +100,7 @@ def __init__(; -199,6 +200,8 @@ def run_moe_core(; symbols: __init__, run_moe_core, forward_deepgemm_masked, forward_cutlass_w4afp8_masked，涉及 `__init__, run_moe_core, forward_deepgemm_masked`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/srt/quant/test_w4a8_deepseek_v3.py`, `test/srt/run_suite.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。

### PR #11877 - [Doc] Add documentation for DeepSeek V3.2

- 链接: https://github.com/sgl-project/sglang/pull/11877
- 状态/时间: merged / 2025-10-25
- 反查来源: `git log --name-only -- <model-files>` 反查到 `docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.md`；关联提交 `729b242934cb`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+723/-3，可读 patch 749 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Doc] Add documentation for DeepSeek V3.2」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.md`；PR 正文摘要: Depends on #11844 and #11876 Also waiting for @whybeyoung on some PD updates。
- 实现要点: `docs/references/multi_node_deployment/rbg_pd/deepseekv32_pd.md` added +570/-0 (570 lines); hunks: -0,0 +1,570。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+15/-0，可读 patch 22 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[doc] add example of using w4fp8 for Deepseek」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `benchmark/deepseek_v3/README.md`；PR 正文摘要: This PR adds a basic example of deploying w4fp8 Deepseek using SGLang. Below are the evaluation of accuracy. | Model | MMLU | |-------|------| | novita/Deepseek-V3.1-W4AFP8 | 0....。
- 实现要点: `benchmark/deepseek_v3/README.md` modified +15/-0 (15 lines); hunks: -368,6 +368,21 @@ edit your `config.json` and remove the `quantization_config...。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+33/-35，可读 patch 233 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support running FP4 Deepseek on SM120.」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/modelopt_quant.py`, `python/sglang/srt/models/gpt_oss.py`, `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: sm120 is not runnable for deepseek because of sm check and invalid kernels. - add `is_blackwell_supported` as an extend to `is_sm100_supported` - bypass dsv3_fused_a_gemm on sm1...。
- 实现要点: `python/sglang/srt/layers/quantization/modelopt_quant.py` modified +8/-7 (15 lines); hunks: -28,7 +28,7; -49,8 +49,10; symbols: apply, ModelOptNvFp4FusedMoEMethod, __init__，涉及 `apply, ModelOptNvFp4FusedMoEMethod, __init__`；`python/sglang/srt/models/gpt_oss.py` modified +1/-10 (11 lines); hunks: -70,18 +70,9；`python/sglang/srt/models/deepseek_v2.py` modified +1/-5 (6 lines); hunks: -131,13 +131,11; -197,8 +195,6; symbols: __init__，涉及 `__init__`；`python/sglang/srt/layers/attention/flashinfer_backend.py` modified +2/-2 (4 lines); hunks: -26,8 +26,8; -229,7 +229,7 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 动机: 标题「[Deepseek V3.2] Enable flashmla_auto with MTP」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`；PR 正文摘要: Follow up to https://github.com/sgl-project/sglang/pull/11655 With fp8 kvcache on B200, flashmla_sparse can be used in the extend (not draft_extend) phases in both normal and sp...。
...
- 动机: 标题「[2/2] Deepseek deterministic: support deepseek v3 deterministic inference on 8 x H200」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: part of roadmap Previous PR supported deepseek arch model's deterministic inference on a single Hopper GPU. This PR is to further support full deepseek v3 model's deterministic...。
...
- 状态/时间: merged / 2025-11-06
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+22/-9，可读 patch 42 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Update dsv3 quantization auto setting for sm100」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/server_args.py`；PR 正文摘要: update quantization based on model quant_method。
- 实现要点: `python/sglang/srt/server_args.py` modified +22/-9 (31 lines); hunks: -912,19 +912,32 @@ def _handle_model_specific_adjustments(self):; symbols: _handle_model_specific_adjustments，涉及 `_handle_model_specific_adjustments`。
- 代码 diff 细节:
...
- 动机: 标题「[Test] Add DeepSeekV3.2 NSA Indexer Test Suite」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`；PR 正文摘要: Address the issue https://github.com/sgl-project/sglang/issues/12509 Added 11 tests: 1. test_forward_decode_mode 2. test_forward_extend_mode 3. test_indexer_basic_creation 4. te...。
...
- 链接: https://github.com/sgl-project/sglang/pull/12689
- 状态/时间: merged / 2025-11-11
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `4a78031a71dd`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 6 个文件，+331/-22，可读 patch 474 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[ROCM] Optimized deepseek-r1 model with rmsnorm + fp8 quant fusion」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Fusion of rmsnorm + group scale FP8 quantization (group_size=128) w/o fusion w/ fusion deepseek.py - LayerCommunicator (for hidden states) - DeepseekV2AttentionMLA (for q, k_) f...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +117/-15 (132 lines); hunks: -153,6 +153,8; -1507,13 +1509,14 @@ def forward_normal_prepare(; symbols: forward_normal_prepare, forward_absorb_prepare, forward_npu_sparse_prepare，涉及 `forward_normal_prepare, forward_absorb_prepare, forward_npu_sparse_prepare`。
- 代码 diff 细节:
...
- 动机: 标题「[DeepseekV32]: use `_concat_mla_absorb_q_general` to replace `torch.cat`」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`；PR 正文摘要: https://github.com/sgl-project/sglang/issues/11989 `torch.cat([q_nope, q_rope], dim=-1)` is heavily used in `nsa_backend`, which is less efficient. Use existed kernel `_concat_m...。
...
- 动机: 标题「[Tool Call] Steamline function arguments when tool_choice="auto" for deepseekv31_detector」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/function_call/deepseekv31_detector.py`；PR 正文摘要: The current `deepseekv31_detector` only detects complete function arguments and immediately returns them while clearing the buffer. This approach can lead to a poor user experie...。
...
- 动机: 标题「[Deepseek V3.2] Use torch.compile to speed up torch.cat in nsa」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`；PR 正文摘要: Use torch.compile to speed up torch.cat used in prefill. Replace the cat ops for k in `forward_extend` in the nsa attention with the torch.compiled version. before after。
...
- 链接: https://github.com/sgl-project/sglang/pull/13264
- 状态/时间: merged / 2025-11-18
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `e389f91decda`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+117/-4，可读 patch 171 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[NVIDIA] Fix broken fp8 MoE of deepseek v3」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This breaks the fp8 moe of the deepseek v3, causing: the root cause is now the routing_method_type becomes a default attribute of FusedMoE layer. So, we should explicitly set it...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +2/-0 (2 lines); hunks: -93,6 +93,7; -674,6 +675,7 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 已读文件:
  - runtime: `python/sglang/srt/models/deepseek_v2.py` modified +2/-0
- 验证与风险: diff 自带测试面 `test/srt/run_suite.py`, `test/srt/test_deepseek_r1_fp8_trtllm_backend.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Fix] Fix DeepSeek V3 MTP on B200」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_nextn.py`；PR 正文摘要: Fix the bug here https://github.com/sgl-project/sglang/actions/runs/19449452397/job/55665254802 Test with GSM8K。
- 实现要点: `python/sglang/srt/models/deepseek_nextn.py` modified +1/-0 (1 lines); hunks: -220,6 +220,7 @@ def __init__(; symbols: __init__, forward，涉及 `__init__, forward`。
- 代码 diff 细节:
...
- 动机: 标题「[ROCM] Optimized deepseek-r1 fp8 model with + triton_gemm_a8w8 + batch_gemm_a8w8 + fused set_mla_kv_buffer kernel」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Co-author : @yichiche To optimize the deepseek-r1 model performance on ROCM. This PR improves the performance of a8w8 GEMM and enable batched_gemm and mla_kv_buffer feature on M...。
...
- 状态/时间: merged / 2025-11-20
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `7291c72e575d`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+92/-124，可读 patch 345 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Deepseek V3.2] Change indexer weights_proj to fp32」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Following https://github.com/deepseek-ai/DeepSeek-V3.2-Exp/commit/8631a813356d39b09a5c1ee1cde8ed6015559eb3, change the indexer `weights_proj` precision from bf16 to fp32. https:...。
...
- 链接: https://github.com/sgl-project/sglang/pull/13705
- 状态/时间: merged / 2025-11-21
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `eff7df6d0a49`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+92/-26，可读 patch 162 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] Enable fused shared expert append and flatten quant for fp8 deepseekR1 model」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Co-author : @yctseng0211 This PR introduces two performance improvements for DeepseekR1-0528 (fp8): 1. Support aiter fused_flatten_fp8_group_quant for fp8 DeepSeek model Adds a...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +10/-1 (11 lines); hunks: -168,10 +168,14; -2001,6 +2005,11 @@ def forward_absorb_core(; symbols: forward_absorb_core，涉及 `forward_absorb_core`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/moe/fused_moe_triton/fused_moe_triton_kernels.py`, `python/sglang/srt/layers/moe/topk.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+253/-77，可读 patch 488 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Perf] Optimize DeepSeek-R1 w4afp8 glue kernels」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/quantization/w4afp8.py`；PR 正文摘要: This PR improves DeepSeek-R1 w4afp8 TP8/EP8 ITL performance. It is motivated by the following profiling trace, obtained by running DeepSeek-R1 w4afp8 TP8 with a concurrency leve...。
- 实现要点: `python/sglang/srt/layers/moe/ep_moe/kernels.py` modified +227/-54 (281 lines); hunks: -16,6 +16,60; -142,25 +196,17 @@ def compute_seg_indptr_triton_kernel(reorder_topk_ids, seg...; symbols: _get_launch_config_1d, get_num_blocks, _get_launch_config_2d, deepep_permute_triton_kernel，涉及 `_get_launch_config_1d, get_num_blocks, _get_launch_config_2d`；`python/sglang/srt/layers/moe/cutlass_w4a8_moe.py` modified +21/-17 (38 lines); hunks: -10,15 +10,17; -44,6 +46,7 @@ def cutlass_w4a8_moe(; symbols: cutlass_w4a8_moe，涉及 `cutlass_w4a8_moe`；`python/sglang/srt/layers/quantization/w4afp8.py` modified +5/-6 (11 lines); hunks: -270,17 +270,17 @@ def process_weights_after_loading(self, layer: Module) ->...; -324,9 +324,8 @@ def apply(; symbols: process_weights_after_loading, apply, apply_deepep_ll，涉及 `process_weights_after_loading, apply, apply_deepep_ll`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/quantization/w4afp8.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix Deepseek v3.1 loading issue」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Fix nightly test error in https://github.com/sgl-project/sglang/actions/runs/19689716521/job/56402800532。
...
- 状态/时间: open / 2025-12-01
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 24 个文件，+1363/-86，可读 patch 2098 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[feature] implement dcp for deepseek_v2」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/utils.py`, `python/sglang/srt/model_executor/forward_batch_info.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`；PR 正文摘要: Here's the first step to fully implement #12196 to support much longer context with TP 8 under 8xH20. Currently, it only works with attention backend flashinfer. It's compatible...。
- 实现要点: `python/sglang/srt/layers/attention/utils.py` modified +254/-0 (254 lines); hunks: -1,7 +1,13; -1391,3 +1397,251 @@ def fused_qk_rope_reshape_and_cache(; symbols: fused_qk_rope_reshape_and_cache, _correct_attn_cp_out_kernel, CPTritonContext, __init__，涉及 `fused_qk_rope_reshape_and_cache, _correct_attn_cp_out_kernel, CPTritonContext`；`python/sglang/srt/model_executor/forward_batch_info.py` modified +136/-0 (136 lines); hunks: -39,6 +39,8; -54,6 +56,7; symbols: ForwardBatch, init_new, _clamp_position_native, create_dcp_kv_indices，涉及 `ForwardBatch, init_new, _clamp_position_native`；`python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py` modified +93/-14 (107 lines); hunks: -1,12 +1,21; -28,6 +37,8; symbols: forward_absorb_prepare, forward_absorb_core，涉及 `forward_absorb_prepare, forward_absorb_core`；`python/sglang/srt/layers/attention/flashinfer_mla_backend.py` modified +83/-7 (90 lines); hunks: -16,11 +16,13; -334,6 +336,7 @@ def init_forward_metadata(self, forward_batch: ForwardBatch):; symbols: init_forward_metadata, init_forward_metadata_replay_cuda_graph, forward_extend, forward_decode，涉及 `init_forward_metadata, init_forward_metadata_replay_cuda_graph, forward_extend`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/distributed/device_communicators/pynccl_allocator.py`, `python/sglang/srt/distributed/parallel_state.py`, `python/sglang/srt/entrypoints/engine.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 链接: https://github.com/sgl-project/sglang/pull/13115
- 状态/时间: merged / 2025-12-06
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `ea177372bd8c`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 11 个文件，+123/-59，可读 patch 459 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「support mtp with deepseek r1 nvfp4 model」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: collabrate with @trevor-m Support large scale EP deployment for the DS R1 fp4 model with eagle spec decoding. - add the custom moe a2a backend for speculative decoding - fix the...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +3/-1 (4 lines); hunks: -2502,7 +2502,9 @@ def forward_normal_chunked_kv_prepare(; symbols: forward_normal_chunked_kv_prepare, forward_normal_chunked_kv_core，涉及 `forward_normal_chunked_kv_prepare, forward_normal_chunked_kv_core`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/attention/trtllm_mla_backend.py`, `python/sglang/srt/layers/moe/utils.py`, `python/sglang/srt/model_executor/forward_batch_info.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 动机: 标题「[Tool Call] Fix DeepSeekV32Detector skipping functions with no params in streaming mode」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/function_call/deepseekv32_detector.py`；PR 正文摘要: When using `--tool-call-parser deepseekv32` in streaming mode, tool calls for functions with no parameters (e.g., `get_date()`) were being silently skipped, while non-streaming...。
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-1，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Auto Sync] Update tool_chat_template_deepseekv31.jinja (20251210)」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `examples/chat_template/tool_chat_template_deepseekv31.jinja`；PR 正文摘要: Sync changes from commit `abd3e13d`. **Files Changed:** - examples/chat_template/tool_chat_template_deepseekv31.jinja Author: Jue Wang *This is an automated PR created by script...。
...
- 状态/时间: merged / 2025-12-11
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+8/-2，可读 patch 21 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix dsv3 dp accuracy issue when using bf16-kv」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/attention/aiter_backend.py`；PR 正文摘要: cherry-pick "Fix dp issue dsv3 bf16 kv #14895" to `amd_mori` branch。
- 实现要点: `python/sglang/srt/layers/attention/aiter_backend.py` modified +8/-2 (10 lines); hunks: -178,12 +178,18 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/14975
- 状态/时间: merged / 2025-12-16
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `8ac350f335c6`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+11/-1，可读 patch 19 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] Support fused_rms_mxfp4_quant in the prefill stage for DeepSeek-R1-MXFP4」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Adopted the fused kernel fused_rms_mxfp4_quant to replace the existing RMSNorm + quant path, enabling more efficient preparation of activations for quantized projections. While...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +11/-1 (12 lines); hunks: -1722,7 +1722,17 @@ def forward_normal_prepare(; symbols: forward_normal_prepare，涉及 `forward_normal_prepare`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/15315
- 状态/时间: open / 2025-12-17
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+58/-11，可读 patch 205 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Performance] Optimize group gemm in DeepSeek-R1-W4AFP8 w4a8 moe」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/scaled_mm_entry.cu`；PR 正文摘要: While deploying and optimizing the w4a8 moe group gemm for DeepSeek-R1-W4AFP8, I found that when w4a8 is in deepep low_latency mode, only the final else branch of the groupgemm...。
- 实现要点: `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py` modified +6/-0 (6 lines); hunks: -170,6 +170,7 @@ def cutlass_w4a8_moe(; -193,6 +194,7 @@ def cutlass_w4a8_moe(; symbols: cutlass_w4a8_moe, cutlass_w4a8_moe_deepep_normal, cutlass_w4a8_moe_deepep_ll，涉及 `cutlass_w4a8_moe, cutlass_w4a8_moe_deepep_normal, cutlass_w4a8_moe_deepep_ll`；`sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu` modified +38/-6 (44 lines); hunks: -79,10 +79,12 @@ void dispatch_w4a8_moe_mm_sm90(; -114,6 +116,20 @@ void dispatch_w4a8_moe_mm_sm90(；`sgl-kernel/csrc/moe/cutlass_moe/w4a8/scaled_mm_entry.cu` modified +6/-3 (9 lines); hunks: -23,7 +23,8 @@ void cutlass_w4a8_moe_mm_sm90(; -49,7 +50,8 @@ void cutlass_w4a8_moe_mm(；`sgl-kernel/python/sgl_kernel/cutlass_moe.py` modified +4/-0 (4 lines); hunks: -57,6 +57,7 @@ def cutlass_w4a8_moe_mm(; -83,6 +84,8 @@ def cutlass_w4a8_moe_mm(; symbols: cutlass_w4a8_moe_mm，涉及 `cutlass_w4a8_moe_mm`。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+2/-0，可读 patch 15 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix the accuracy issue when running mxfp4 dsv3 model and enable ep」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/quantization/mxfp4.py`, `python/sglang/srt/layers/quantization/quark/quark_moe.py`；PR 正文摘要: Fix the accuracy problem Add expert mask in moe function call `export SGLANG_AITER_MLA_PERSIST=0 python3 -m sglang.launch_server \ --model-path $MODEL \ --tp-size 8 \ --trust-re...。
- 实现要点: `python/sglang/srt/layers/quantization/mxfp4.py` modified +1/-0 (1 lines); hunks: -866,5 +866,6 @@ def apply(; symbols: apply，涉及 `apply`；`python/sglang/srt/layers/quantization/quark/quark_moe.py` modified +1/-0 (1 lines); hunks: -245,6 +245,7 @@ def apply(; symbols: apply，涉及 `apply`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/quantization/mxfp4.py`, `python/sglang/srt/layers/quantization/quark/quark_moe.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。

### PR #15307 - [Deepseek V3.2] Support Overlap Spec + NSA

- 链接: https://github.com/sgl-project/sglang/pull/15307
- 状态/时间: merged / 2025-12-17
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `python/sglang/srt/layers/attention/nsa_backend.py`；关联提交 `d20699a33c50`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+25/-8，可读 patch 82 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Deepseek V3.2] Support Overlap Spec + NSA」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；PR 正文摘要: Part of V3.2 Roadmap https://github.com/sgl-project/sglang/issues/15025 Enable overlap spec and EAGLE + NSA backend. In EAGLE V1, we had (with `python3 -m sglang.test.send_one -...。
...
- 动机: 标题「feat: DeepSeek-V3.2 Streaming tool call output」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/function_call/deepseekv32_detector.py`；PR 正文摘要: FIxes #14711。
...
- 状态/时间: merged / 2025-12-18
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+160/-264，可读 patch 516 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[perf]optimize w4afp8 kernel on deepseek-v3-0324」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/w4afp8.py`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu`, `sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_moe_data.cu`；PR 正文摘要: we use w4afp8 deepseekv3-0324 online, and we find its performance is not good enough when decode batch size < 32 fine-grained tiling config and based on https://github.com/sgl-p...。
- 实现要点: `python/sglang/srt/layers/quantization/w4afp8.py` modified +0/-1 (1 lines); hunks: -300,7 +300,6 @@ def apply(; symbols: apply，涉及 `apply`；`sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_grouped_mm_c3x.cu` modified +64/-236 (300 lines); hunks: -66,6 +66,25 @@ inline void invoke_gemm(; -87,268 +106,77 @@ void dispatch_w4a8_moe_mm_sm90(；`sgl-kernel/csrc/moe/cutlass_moe/w4a8/w4a8_moe_data.cu` modified +96/-27 (123 lines); hunks: -2,29 +2,48; -34,20 +53,69 @@ __global__ void compute_problem_sizes_w4a8(。
- 代码 diff 细节:
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Deepseek V3.2] Fix Deepseek MTP in V1 mode」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`；PR 正文摘要: Restore the earlier behaviour of Deepseek V3 + MTP. (Accidental change) Fix https://github.com/sgl-project/sglang/issues/15428。
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `8fe3e3746832`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 7 个文件，+148/-16，可读 patch 261 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Support piecewise cuda graph for dsv3 fp4」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: https://github.com/sgl-project/sglang/issues/11490。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +11/-1 (12 lines); hunks: -20,6 +20,7; -400,6 +401,9 @@ def handle_attention_fa4(attn, forward_batch):; symbols: handle_attention_fa4, handle_attention_trtllm_mla, forward，涉及 `handle_attention_fa4, handle_attention_trtllm_mla, forward`。
- 代码 diff 细节:
...
- 已读文件:
  - runtime: `python/sglang/srt/models/deepseek_v2.py` modified +11/-1
- 验证与风险: diff 自带测试面 `test/srt/run_suite.py`, `test/srt/test_deepseek_v3_fp4_4gpu.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 动机: 标题「[Tool Call][DSV32] Streamline function call parameters」；模型线: DeepSeek V3/R1；类别: 文档/测试/CI；主要 diff: `python/sglang/srt/function_call/deepseekv32_detector.py`；PR 正文摘要: As titled and Issue #14711. **IMPORTANT: This is an initial version aimed at streamlining function call parameters for `DeepSeek-V3.2`. I have tested it, and it passes all tests...。
...
- 链接: https://github.com/sgl-project/sglang/pull/14280
- 状态/时间: merged / 2025-12-29
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `2ec6fa3c54a8`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+12/-1，可读 patch 34 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「feat PD: add eagle3 support for DeepSeek V3 in EP mode」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: When try to enable EP MOE for DeepSeek V3 with EAGLE3, SGL_ENABLE_JIT_DEEPGEMM_BMM=1 SGL_ENABLE_JIT_DEEPGEMM=1 \ TORCHINDUCTOR_CACHE_DIR=/home/admin/inductor_root_cache /opt/con...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +12/-1 (13 lines); hunks: -46,6 +46,7; -3117,6 +3118,10 @@ def __init__(; symbols: __init__, forward，涉及 `__init__, forward`。
- 代码 diff 细节:
...
- 动机: 标题「Fix DeepSeekV31's structural tag trigger」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/function_call/deepseekv31_detector.py`；PR 正文摘要: As titled. To prevent LLMs generating tool calls with wrong name Reference (can be seen in other models in the SGLang codebase as well):。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+721/-600，可读 patch 1455 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Refactor] Split out deepseek v2 weight loader function into mixin」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/utils.py`；PR 正文摘要: DeepseekV2 code has been developed fast and a lot of historical code and be more orgranized, including the weight loading part. Issue related: https://github.com/sgl-project/sgl...。
- 实现要点: `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py` added +657/-0 (657 lines); hunks: -0,0 +1,657; symbols: DeepseekV2WeightLoaderMixin, do_load_weights, post_load_weights, _maybe_quant_weights_to_fp8_ue8m0，涉及 `DeepseekV2WeightLoaderMixin, do_load_weights, post_load_weights`；`python/sglang/srt/models/deepseek_v2.py` modified +9/-594 (603 lines); hunks: -17,15 +17,13; -111,44 +109,29; symbols: enable_nextn_moe_bf16_cast_to_fp8, forward, DeepseekV2ForCausalLM, __init__，涉及 `enable_nextn_moe_bf16_cast_to_fp8, forward, DeepseekV2ForCausalLM`；`python/sglang/srt/models/deepseek_common/utils.py` modified +53/-1 (54 lines); hunks: -1,3 +1,19; -19,5 +35,41; symbols: awq_dequantize_func, enable_nextn_moe_bf16_cast_to_fp8，涉及 `awq_dequantize_func, enable_nextn_moe_bf16_cast_to_fp8`；`python/sglang/srt/models/deepseek_nextn.py` modified +2/-5 (7 lines); hunks: -46,11 +46,8。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/models/deepseek_common/utils.py`, `python/sglang/srt/models/deepseek_nextn.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 动机: 标题「Use dsv3 optimized routing `fused_topk_deepseek` instead of `moe_fused_gate`」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/topk.py`, `test/registered/kernels/test_fused_topk_deepseek.py`, `test/srt/test_deepseek_v3_mtp.py`；PR 正文摘要: flashinfer has an optimized routing kernel for DeepSeek V3: https://github.com/flashinfer-ai/flashinfer/pull/2099 The API was renamed to `fused_topk_deepseek` here: https://gith...。
...
- 动机: 标题「[DeepSeek v3.2] Opt MTP decode cuda batch sizes and nsa implementation」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`；PR 正文摘要: 1、`draft_extend` and `target_verify` use `nsa_decode_backend` as backend implementation instead of `nsa_prefill_backend` In the MTP scenario, the NSA attention implementation se...。
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_perf_mi35x.py`；关联提交 `a3addd6203ca`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 18 个文件，+1140/-99，可读 patch 1539 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] Add DeepSeek-V3.2 and VLMs model in nightly tests」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_perf_mi35x.py`；PR 正文摘要: Add 5 unique test in AMD coverage: https://github.com/sgl-project/sglang/actions/runs/21153414554/attempts/1#summary-60833755716 - Add accuracy and perf tests for the DeepSeek-V...。
- 实现要点: `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py` modified +5/-30 (35 lines); hunks: -1,7 +1,7; -32,9 +32,9; symbols: get_mxfp4_models，涉及 `get_mxfp4_models`；`test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_perf_mi35x.py` modified +13/-3 (16 lines); hunks: -30,7 +30,10; -43,7 +46,14 @@ def generate_simple_markdown_report(results: List[BenchmarkRe...; symbols: generate_simple_markdown_report, setUpClass，涉及 `generate_simple_markdown_report, setUpClass`。
- 代码 diff 细节:
...
- 动机: 标题「[OPT] DeepSeekV3.2: optimize indexer weight_proj-mma performance」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；PR 正文摘要: From these pr[https://github.com/sgl-project/sglang/pull/16637 and https://github.com/sgl-project/sglang/issues/16861 and https://github.com/sgl-project/sglang/pull/13459/files]...。
...
- 动机: 标题「[DeepSeek V3.2] Enable trtllm NSA with bf16 kvcache」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`；PR 正文摘要: Enables NSA backend with trtllm kernels for sparse attention. This can be more efficient than FlashMLA when the head size isn't a multiple of 64 and hence requires padding. This...。
...
- 动机: 标题「[DSv32] Overlap indexer qk projection and activation quant」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；PR 正文摘要: @humansand After https://github.com/sgl-project/sglang/pull/17205, the indexer weight projection is fully hidden and no longer exposes latency. Per layer, there is a (52.5-39.5)...。
...
- 状态/时间: merged / 2026-01-28
- 反查来源: `git log --name-only -- <model-files>` 反查到 `test/registered/amd/accuracy/mi35x/test_deepseek_r1_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`；关联提交 `f8636fbb253a`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 27 个文件，+1540/-43，可读 patch 1823 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] Add Kimi-K2, DeepSeek-V3.2 tests to nightly CI」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `test/registered/amd/accuracy/mi35x/test_deepseek_r1_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py`；PR 正文摘要: 1. Add **Kimi-K2**, **DeepSeek-V3.2** accuracy and performance tests for MI325 (MI30x) platform, update Mi35x tests, consolidate test jobs, and fix various CI failures. 2. Total...。
- 实现要点: `test/registered/amd/accuracy/mi35x/test_deepseek_r1_eval_mi35x.py` modified +3/-0 (3 lines); hunks: -214,6 +214,9 @@ def test_deepseek_r1_accuracy(self):; symbols: test_deepseek_r1_accuracy，涉及 `test_deepseek_r1_accuracy`；`test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_eval_mi35x.py` modified +3/-0 (3 lines); hunks: -239,6 +239,9 @@ def test_deepseek_r1_mxfp4_accuracy(self):; symbols: test_deepseek_r1_mxfp4_accuracy，涉及 `test_deepseek_r1_mxfp4_accuracy`。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+16/-12，可读 patch 43 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix OOM in DeepSeek weight loading by deferring dict(weights) materialization」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`；PR 正文摘要: - Fix OOM when loading DeepSeek models with streaming/remote weight loading - Defer `dict(weights)` call until quantization is actually needed - Return original weights iterator...。
- 实现要点: `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py` modified +16/-12 (28 lines); hunks: -623,9 +623,9 @@ def _maybe_quant_weights_to_fp8_ue8m0(; -655,16 +655,20 @@ def _maybe_quant_weights_to_fp8_ue8m0(; symbols: _maybe_quant_weights_to_fp8_ue8m0，涉及 `_maybe_quant_weights_to_fp8_ue8m0`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 动机: 标题「[NPU] support dsv32 radixcache on ascend」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；PR 正文摘要: Ascend currently does not support the radixcache feature on the DeepSeek-3.2 model 1. We have modified the inputs of `npu_sparse_flash_attention` and `npu_lightning_indexer` to...。
...
- 动机: 标题「Tilelang sparse decode fwd for dsv32 mi355」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`；PR 正文摘要: cc @HaiShaw, @kkHuang-amd, @hubertlu-tw The current dsv32 attention implementation uses a single TileLang kernel for both prefill and decode. This kernel is optimized for prefil...。
...
- 动机: 标题「Fix Bug on dsv3.2」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；PR 正文摘要: Fix two bugs on dsv32 when infering on npu 1. fix a bug: forward_npu method of nas_Indexer has a dual stream feature that can not be closed. May cause problem in some cases. 2....。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 2 个文件，+9/-5，可读 patch 45 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] Fix accuracy issue when running TP4 dsv3 model with mtp」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/attention/aiter_backend.py`, `docker/rocm.Dockerfile`；PR 正文摘要: cc @HaiShaw, @kkHuang-amd Fixed dpsk-r1-fp4 accuracy issue on tp4 with mtp. - aiter backend change in sglang. - aiter update for tp4 fp8 prefill attn kernel (head=32). (merged,...。
- 实现要点: `python/sglang/srt/layers/attention/aiter_backend.py` modified +7/-3 (10 lines); hunks: -195,11 +195,15 @@ def __init__(; -301,7 +305,7 @@ def make_mla_meta_data(; symbols: __init__, make_mla_meta_data，涉及 `__init__, make_mla_meta_data`；`docker/rocm.Dockerfile` modified +2/-2 (4 lines); hunks: -21,7 +21,7 @@ ENV BUILD_TRITON="0"; -31,7 +31,7 @@ ENV BUILD_TRITON="0"。
- 代码 diff 细节:
...
- 动机: 标题「Fix model loading for DeepSeek-V3.2-AWQ」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Got the following error when using DeepSeek-V3.2-AWQ (https://huggingface.co/QuantTrio/DeepSeek-V3.2-AWQ/) Add check of whether `weight_packed` exists before using Tested on gsm...。
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/registered/kernels/test_nsa_indexer.py`；关联提交 `0ffd0a3995e5`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 10 个文件，+352/-183，可读 patch 914 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Nsa trtllm mla sparse fp8 support with Deepseek v3.2 NVFP4」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`, `python/sglang/srt/models/deepseek_v2.py`, `test/registered/kernels/test_nsa_indexer.py`；PR 正文摘要: 17655 - support Deepseek v3.2 NVFP4 with trtllm mla sparse fp8 attention backend - update the nsa backend to support trtllm sparse fp8 attention backend - update the deepseek v2...。
- 实现要点: `python/sglang/srt/layers/attention/nsa_backend.py` modified +172/-66 (238 lines); hunks: -33,7 +33,10; -340,6 +343,7 @@ def __init__(; symbols: __init__, forward_extend，涉及 `__init__, forward_extend`；`python/sglang/srt/models/deepseek_v2.py` modified +6/-0 (6 lines); hunks: -1488,6 +1488,12 @@ def _fuse_rope_for_trtllm_mla(self, forward_batch: Forwar...; symbols: _fuse_rope_for_trtllm_mla，涉及 `_fuse_rope_for_trtllm_mla`；`test/registered/kernels/test_nsa_indexer.py` modified +1/-0 (1 lines); hunks: -232,6 +232,7 @@ def __init__(self, config=None):; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 链接: https://github.com/sgl-project/sglang/pull/18892
- 状态/时间: open / 2026-02-16
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 9 个文件，+1284/-0，可读 patch 1299 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Kernel] Add JIT support for DeepSeek V3 GEMM」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/jit_kernel/csrc/gemm/dsv3_fused_a_gemm.cuh`, `python/sglang/jit_kernel/csrc/gemm/dsv3_router_gemm.cuh`, `python/sglang/jit_kernel/benchmark/bench_dsv3_router_gemm.py`；PR 正文摘要: This PR migrates DeepSeek V3 GEMM kernels to the JIT (Just-In-Time) compilation framework, including: - `dsv3_fused_a_gemm` - `dsv3_router_gemm` related roadmap: https://github....。
- 实现要点: `python/sglang/jit_kernel/csrc/gemm/dsv3_fused_a_gemm.cuh` added +602/-0 (602 lines); hunks: -0,0 +1,602; symbols: Type，涉及 `Type`；`python/sglang/jit_kernel/csrc/gemm/dsv3_router_gemm.cuh` added +176/-0 (176 lines); hunks: -0,0 +1,176；`python/sglang/jit_kernel/benchmark/bench_dsv3_router_gemm.py` added +118/-0 (118 lines); hunks: -0,0 +1,118; symbols: benchmark_bf16, tflops, benchmark_fp32，涉及 `benchmark_bf16, tflops, benchmark_fp32`；`python/sglang/jit_kernel/benchmark/bench_dsv3_fused_a_gemm.py` added +83/-0 (83 lines); hunks: -0,0 +1,83; symbols: _make_inputs, benchmark, tflops，涉及 `_make_inputs, benchmark, tflops`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `python/sglang/jit_kernel/tests/test_dsv3_fused_a_gemm.py`, `python/sglang/jit_kernel/tests/test_dsv3_router_gemm.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] Fix mi35x dsv32 mtp nightly」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/attention/nsa_backend.py`；PR 正文摘要: PR #17554 introduced `fused_metadata_copy`, a JIT-compiled CUDA kernel enabled by default. This kernel cannot run on ROCm/HIP — it fails on every CUDA graph replay iteration, pr...。
...
- 动机: 标题「[DSv32] [GLM5] Improve Model Quality by Avoiding FP32 Precision Loss in `weights_proj`」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`, `test/registered/kernels/test_nsa_indexer.py`；PR 正文摘要: @humansand From recent GLM-5 tech report: > Compared with the non-deterministic CUDA-based top‑k implementation used in SGLang’s DSA Indexer, directly using the naive torch.topk...。
...
- 链接: https://github.com/sgl-project/sglang/pull/18624
- 状态/时间: merged / 2026-02-24
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `c193a52fa263`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+13/-9，可读 patch 67 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] DSR1/V3 use fp8 bmm in MLA for MI300X」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Improve TPOT by using use fp8 bmm in MLA on MI300X for DSR1/V3 bs=16, ISL 8k OSL 1K Before **q@w_kc: bmm** **attn_output@w_vc: bmm** After **q@w_kc: bmm** **attn_output@w_vc: bmm**。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +13/-9 (22 lines); hunks: -119,6 +119,7; -158,11 +159,11; symbols: forward, forward_absorb_prepare, forward_absorb_core，涉及 `forward, forward_absorb_prepare, forward_absorb_core`。
- 代码 diff 细节:
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `b9cf1563de97`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+7/-2，可读 patch 44 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[ROCm] Optimize Deepseek R1 on MI300X」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: The DeepSeek-V3/R1 model on AMD MI300X (gfx942) was previously limited to fallback code paths for several MLA attention operations because the optimized kernels were gated behin...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +5/-2 (7 lines); hunks: -180,6 +180,9; -1724,7 +1727,7 @@ def forward_absorb_prepare(; symbols: forward_absorb_prepare, forward_absorb_core，涉及 `forward_absorb_prepare, forward_absorb_core`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/attention/aiter_backend.py`, `python/sglang/srt/layers/quantization/fp8_utils.py`, `python/sglang/srt/models/deepseek_v2.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 动机: 标题「[AMD] Fix weight load shape mismatch for amd dsr1 0528 mxfp4」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_nextn.py`, `python/sglang/srt/layers/quantization/quark/quark.py`；PR 正文摘要: > Co-authored-by: Haoyang Li > Co-authored-by: billishyahao This patch is to fix tensor shape mismatch issue for amd dsr1 0528 mxfp4 https://huggingface.co/amd/DeepSeek-R1-0528-...。
...
- 动机: 标题「[Bugfix] Catch errors when DeepSeek-V3.2 generates malformed JSON」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/function_call/deepseekv32_detector.py`；PR 正文摘要: LLMs may generate tool call content that are not correct json format, we must catch this error for normal streaming output.。
...
- 动机: 标题「[Tool Call] Stream DeepSeek-V3.2 function call parameters in JSON format.」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/function_call/deepseekv32_detector.py`；PR 正文摘要: The previous implementation only supported streaming function call parameters in XML format. 1. Convert `_parse_parameters_from_xml` to return `str`, unifying the handling of JS...。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 13 个文件，+1614/-177，可读 patch 2530 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] CI - Add MI35x nightly/PR tests for kv-cache-fp8 and allreduce-fusion (DeepSeek)」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_kv_fp8_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_ar_fusion_eval_mi35x.py`, `.github/workflows/nightly-test-amd-rocm720.yml`；PR 正文摘要: Track accuracy and performance regression for two new DeepSeek-R1-MXFP4 server configurations on MI35x: - `--kv-cache-dtype fp8_e4m3` - `--enable-aiter-allreduce-fusion` Update...。
- 实现要点: `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_kv_fp8_eval_mi35x.py` added +281/-0 (281 lines); hunks: -0,0 +1,281; symbols: get_model_path, ModelConfig, __post_init__, get_display_name，涉及 `get_model_path, ModelConfig, __post_init__`；`test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_ar_fusion_eval_mi35x.py` added +280/-0 (280 lines); hunks: -0,0 +1,280; symbols: get_model_path, ModelConfig, __post_init__, get_display_name，涉及 `get_model_path, ModelConfig, __post_init__`；`.github/workflows/nightly-test-amd-rocm720.yml` modified +153/-68 (221 lines); hunks: -21,46 +21,10 @@ on:; -98,7 +62,7 @@ jobs:；`.github/workflows/nightly-test-amd.yml` modified +153/-68 (221 lines); hunks: -21,46 +21,10 @@ on:; -98,7 +62,7 @@ jobs:。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_ar_fusion_eval_mi35x.py`, `test/registered/amd/accuracy/mi35x/test_deepseek_r1_mxfp4_kv_fp8_eval_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_ar_fusion_perf_mi35x.py`, `test/registered/amd/perf/mi35x/test_deepseek_r1_mxfp4_kv_fp8_perf_mi35x.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `84aaa69795f3`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+12/-7，可读 patch 26 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] Use bfloat16 for correction_bias in AITER FP8 path to avoid runtime dtype conversion for dsv3」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: In the AITER (AMD MI355X) path with FP8 quantization, was initialized in float32, but at runtime in biased_grouped_topk it gets cast to match gating_output.dtype: correction_bia...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +12/-7 (19 lines); hunks: -271,13 +271,18 @@ def __init__(; symbols: __init__，涉及 `__init__`。
- 代码 diff 细节:
...
- 动机: 标题「[deepseekv3.2] fix get_k_and_s_triton kenel for 128K seqlen case bug」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `test/manual/layers/attention/nsa/test_get_k_scale_triton_kernel.py`, `python/sglang/srt/layers/attention/nsa/index_buf_accessor.py`, `test/manual/layers/attention/nsa/test_index_buf_accessor.py`；PR 正文摘要: fix get_k_and_s_triton kenel for 128K seqlen case bug。
...
- 动机: 标题「[CPU] Add FP8 Bmm support」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/longcat_flash.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`；PR 正文摘要: This PR is a follow-up on https://github.com/sgl-project/sglang/issues/8281 to add FP8 BMM support in MLA weight absorb. - The main change is the C++ kernels for fp8 bmm on CPU:...。
...
- 状态/时间: merged / 2026-03-19
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_v2.py`；关联提交 `85fe8c6793a0`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+5/-1，可读 patch 13 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[AMD] Use aiter_dsv3_router_gemm kernel if number of experts <= 256.」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: This change modified the router kernel selection with ROCm backend for models using DeepSeek model architecture. The Aiter kernel aiter_dsv3_router_gemm has been optimized for t...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +5/-1 (6 lines); hunks: -325,7 +325,11 @@ def forward(; symbols: forward，涉及 `forward`。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-1，可读 patch 9 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix gpu-fault issue when run deepseek-r1 and enable dp」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/layers/attention/aiter_backend.py`；PR 正文摘要: Fix the gpu cored-dump. Root cause: During CUDA graph capture, the captured data was fed with a KV sequence length smaller than the query (Q) length, which caused an illegal mem...。
- 实现要点: `python/sglang/srt/layers/attention/aiter_backend.py` modified +1/-1 (2 lines); hunks: -1774,7 +1774,7 @@ def init_forward_metadata_replay_cuda_graph(; symbols: init_forward_metadata_replay_cuda_graph, get_cuda_graph_seq_len_fill_value, update_verify_buffers_to_fill_after_draft，涉及 `init_forward_metadata_replay_cuda_graph, get_cuda_graph_seq_len_fill_value, update_verify_buffers_to_fill_after_draft`。
- 代码 diff 细节:
...
- 动机: 标题「[Perf] Overlap NSA-CP key all-gather with query computation for DeepSeek-V3.2」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa/nsa_indexer.py`；PR 正文摘要: In the current Native Sparse Attention (NSA) implementation, specifically when Context Parallelism (CP) is enabled via `--enable-nsa-prefill-context-parallel`, the key_all_gathe...。
...
- 动机: 标题「[AMD] Tilelang sparse fwd for dsv32 mi355/mi300」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/attention/nsa/tilelang_kernel.py`；PR 正文摘要: - Enable the faster/new tilelang kernel on MI300. - Improve longer-context kernel performance on MI355. - Enable the new tilelang partial+combine path with MI300 config: block_I...。
...
- 链接: https://github.com/sgl-project/sglang/pull/21526
- 状态/时间: open / 2026-03-27
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 1 个文件，+1/-0，可读 patch 8 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Fix aiter router GEMM regression for non-DSR1 MoE models on ROCm gfx95」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: aiter_dsv3_router_gemm uses a Triton atomic kernel (_gemm_a16_w16_atomic) that is specifically tuned for DeepSeek-R1/V3's hidden_size=7168. On models with different hidden dimen...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +1/-0 (1 lines); hunks: -330,6 +330,7 @@ def forward(; symbols: forward，涉及 `forward`。
- 代码 diff 细节:
...
- 状态/时间: open / 2026-03-27
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 11 个文件，+450/-39，可读 patch 560 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[JIT Kernel] Migrate dsv3_router_gemm from AOT sgl-kernel to JIT kernel」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/glm4_moe_lite.py`, `python/sglang/jit_kernel/csrc/gemm/dsv3_router_gemm.cuh`；PR 正文摘要: dsv3_router_gemm_bf16_out.cu → kOutFloat=false path dsv3_router_gemm_float_out.cu → kOutFloat=true path dsv3_router_gemm_entry.cu → RouterGemmDispatcher replace LoopUnroller htt...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +13/-6 (19 lines); hunks: -162,7 +162,14; -317,13 +324,13 @@ def forward(; symbols: forward，涉及 `forward`；`python/sglang/srt/models/glm4_moe_lite.py` modified +10/-5 (15 lines); hunks: -181,13 +181,18 @@ def forward(self, hidden_states, gemm_output_zero_allocato...; symbols: forward，涉及 `forward`；`python/sglang/jit_kernel/csrc/gemm/dsv3_router_gemm.cuh` added +184/-0 (184 lines); hunks: -0,0 +1,184；`python/sglang/jit_kernel/benchmark/bench_dsv3_router_gemm.py` added +104/-0 (104 lines); hunks: -0,0 +1,104; symbols: benchmark_bf16_output, benchmark_float32_output，涉及 `benchmark_bf16_output, benchmark_float32_output`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `python/sglang/jit_kernel/tests/test_dsv3_router_gemm.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 动机: 标题「[Intel GPU] Enable DeepSeek R1 inference on XPU」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/models/deepseek_common/utils.py`, `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: Enable DeepSeek-R1 model inference for XPU use FP8 precision through triton 1. run bmm in bf16 for absorb 2. Add XPU support for benchmarking 3. Add EP support Not Applicable No...。
...
- 状态/时间: merged / 2026-03-30
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+94/-12，可读 patch 187 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`；PR 正文摘要: profiling: deepseek-R1-0528: deepseek-R1-0528-w4afp8: 1.When DeepEP is enabled, the communication latency of the DeepSeek-R1-0508-W4AFP8 model is twice that of the DeepSeek-R1-0...。
- 实现要点: `python/sglang/srt/layers/moe/ep_moe/kernels.py` modified +73/-0 (73 lines); hunks: -1381,3 +1381,76 @@ def silu_and_mul_masked_post_per_tensor_quant_fwd(; symbols: silu_and_mul_masked_post_per_tensor_quant_fwd, _fp8_per_token_quant_to_per_tensor_quant_kernel, fp8_per_token_to_per_tensor_quant_triton，涉及 `silu_and_mul_masked_post_per_tensor_quant_fwd, _fp8_per_token_quant_to_per_tensor_quant_kernel, fp8_per_token_to_per_tensor_quant_triton`；`python/sglang/srt/layers/moe/cutlass_w4a8_moe.py` modified +15/-7 (22 lines); hunks: -25,6 +25,7; -411,7 +412,8 @@ def cutlass_w4a8_moe_deepep_normal(; symbols: cutlass_w4a8_moe_deepep_normal, cutlass_w4a8_moe_deepep_ll，涉及 `cutlass_w4a8_moe_deepep_normal, cutlass_w4a8_moe_deepep_ll`；`python/sglang/srt/layers/moe/ep_moe/layer.py` modified +3/-3 (6 lines); hunks: -331,6 +331,9 @@ def forward_cutlass_w4afp8(; -342,9 +345,6 @@ def forward_cutlass_w4afp8_masked(; symbols: forward_cutlass_w4afp8, forward_cutlass_w4afp8_masked，涉及 `forward_cutlass_w4afp8, forward_cutlass_w4afp8_masked`；`python/sglang/srt/layers/quantization/w4afp8.py` modified +2/-1 (3 lines); hunks: -334,10 +334,11 @@ def apply_deepep_ll(; symbols: apply_deepep_ll，涉及 `apply_deepep_ll`。
- 代码 diff 细节:
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+12/-94，可读 patch 187 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Revert "DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication"」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`；PR 正文摘要: Reverts sgl-project/sglang#14162。
- 实现要点: `python/sglang/srt/layers/moe/ep_moe/kernels.py` modified +0/-73 (73 lines); hunks: -1381,76 +1381,3 @@ def silu_and_mul_masked_post_per_tensor_quant_fwd(; symbols: silu_and_mul_masked_post_per_tensor_quant_fwd, _fp8_per_token_quant_to_per_tensor_quant_kernel, fp8_per_token_to_per_tensor_quant_triton，涉及 `silu_and_mul_masked_post_per_tensor_quant_fwd, _fp8_per_token_quant_to_per_tensor_quant_kernel, fp8_per_token_to_per_tensor_quant_triton`；`python/sglang/srt/layers/moe/cutlass_w4a8_moe.py` modified +7/-15 (22 lines); hunks: -25,7 +25,6; -412,8 +411,7 @@ def cutlass_w4a8_moe_deepep_normal(; symbols: cutlass_w4a8_moe_deepep_normal, cutlass_w4a8_moe_deepep_ll，涉及 `cutlass_w4a8_moe_deepep_normal, cutlass_w4a8_moe_deepep_ll`；`python/sglang/srt/layers/moe/ep_moe/layer.py` modified +3/-3 (6 lines); hunks: -331,9 +331,6 @@ def forward_cutlass_w4afp8(; -345,6 +342,9 @@ def forward_cutlass_w4afp8_masked(; symbols: forward_cutlass_w4afp8, forward_cutlass_w4afp8_masked，涉及 `forward_cutlass_w4afp8, forward_cutlass_w4afp8_masked`；`python/sglang/srt/layers/quantization/w4afp8.py` modified +1/-2 (3 lines); hunks: -334,11 +334,10 @@ def apply_deepep_ll(; symbols: apply_deepep_ll，涉及 `apply_deepep_ll`。
- 代码 diff 细节:
...
- 动机: 标题「[AMD] Use tgemm.mm for MoEGate router gemm in deepseek_v2.py」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/rocm_linear_utils.py`, `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: When running the GLM-5-fp8 model, the MoEGate router GEMM is a major bottleneck, taking about **~30%** of a single layer's time. Following the ATOM approach, this change switche...。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 3 个文件，+105/-45，可读 patch 204 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[RL] Support mxfp8 DeepSeek V3」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/moe_runner/flashinfer_trtllm.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/layers/quantization/fp8.py`；PR 正文摘要: @humansand Support Blackwell mxfp8 DeepSeek RL. Since the `kv_b_proj` can have different contraction axis in absorbed vs non-absorbed MLA mode while mxfp8 is 1d quantization, fo...。
- 实现要点: `python/sglang/srt/layers/moe/moe_runner/flashinfer_trtllm.py` modified +86/-38 (124 lines); hunks: -47,6 +47,10; -126,10 +130,13 @@ def align_fp8_moe_weights_for_flashinfer_trtllm(; symbols: align_fp8_moe_weights_for_flashinfer_trtllm, align_mxfp8_moe_weights_for_flashinfer_trtllm，涉及 `align_fp8_moe_weights_for_flashinfer_trtllm, align_mxfp8_moe_weights_for_flashinfer_trtllm`；`python/sglang/srt/layers/moe/fused_moe_triton/layer.py` modified +12/-7 (19 lines); hunks: -291,13 +291,18 @@ def __init__(; symbols: __init__，涉及 `__init__`；`python/sglang/srt/layers/quantization/fp8.py` modified +7/-0 (7 lines); hunks: -93,6 +93,7; -241,6 +242,12 @@ def get_quant_method(; symbols: get_quant_method, get_scaled_act_names, apply_weight_name_mapper, Fp8LinearMethod，涉及 `get_quant_method, get_scaled_act_names, apply_weight_name_mapper`。
- 代码 diff 细节:
...
- 动机: 标题「Add dsv3 router gemm benchmark on blackwell」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: * Comparing dsv3_router_gemm performance between flashinfer and the current sglang kernel to optimize performance (https://github.com/sgl-project/sglang/issues/14453) * Add accu...。
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `5a3531641735`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 4 个文件，+196/-20，可读 patch 358 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「Enable IndexCache for DeepSeek V3.2」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`；PR 正文摘要: fix https://github.com/sgl-project/sglang/issues/21286 * Port https://github.com/THUDM/IndexCache * add ut for deepseek-v3.2 gsm8k with DeepSeek-V3.2-Exp with this PR: main Thro...。
...
- 动机: 标题「[Bugfix] Fix prepare_qkv_latent bypassing LoRA adapters in DeepSeek V2/V3」；模型线: DeepSeek V3/R1；类别: 缺陷修复；主要 diff: `python/sglang/srt/models/deepseek_v2.py`；PR 正文摘要: When LoRA adapters are applied to `fused_qkv_a_proj_with_mqa` in DeepSeek V2/V3 models, the fused GEMM optimization path in `prepare_qkv_latent` reads `.weight` directly from th...。
...
- 链接: https://github.com/sgl-project/sglang/pull/20089
- 状态/时间: merged / 2026-04-09
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+199/-49，可读 patch 448 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「feat: [1/2] [DeepEP] Fuse shared expert into MoE dispatch under EP」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/layers/moe/topk.py`, `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`；PR 正文摘要: In DeepSeek V3/R1 with expert parallelism (EP), each rank currently computes the shared expert **separately** from routed experts. This means shared expert is not part of the De...。
- 实现要点: `python/sglang/srt/models/deepseek_v2.py` modified +84/-25 (109 lines); hunks: -103,6 +103,9; -364,11 +367,34 @@ def __init__(; symbols: __init__, forward_deepep，涉及 `__init__, forward_deepep`；`python/sglang/srt/layers/moe/topk.py` modified +82/-8 (90 lines); hunks: -36,7 +36,11; -49,6 +53,7; symbols: biased_grouped_topk_gpu, biased_grouped_topk_cpu, _remap_topk_for_deepep，涉及 `biased_grouped_topk_gpu, biased_grouped_topk_cpu, _remap_topk_for_deepep`；`python/sglang/srt/layers/moe/fused_moe_triton/layer.py` modified +20/-16 (36 lines); hunks: -48,7 +48,7; -197,10 +197,20 @@ def __init__(; symbols: __init__, _load_g_idx, _map_global_expert_id_to_local_expert_id, weight_loader，涉及 `__init__, _load_g_idx, _map_global_expert_id_to_local_expert_id`；`python/sglang/srt/layers/moe/utils.py` modified +6/-0 (6 lines); hunks: -254,6 +254,12 @@ def is_sbo_enabled() -> bool:; symbols: is_sbo_enabled, is_deepep_class_backend, get_tbo_token_distribution_threshold，涉及 `is_sbo_enabled, is_deepep_class_backend, get_tbo_token_distribution_threshold`。
- 代码 diff 细节:
...
- 验证与风险: runtime 路径改动集中在 `python/sglang/srt/layers/moe/fused_moe_triton/layer.py`, `python/sglang/srt/layers/moe/topk.py`, `python/sglang/srt/layers/moe/utils.py`；风险点是权重加载、并行切分、attention/MoE 后端和 parser 输出，需要至少做一次真实 checkpoint 或等价 mock smoke。
...
- 状态/时间: merged / 2026-04-09
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 16 个文件，+458/-80，可读 patch 811 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Lora] Lora quat info re-factor and support deepseekv3 mla lora」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/quantization/fp8.py`, `python/sglang/srt/layers/quantization/moe_wna16.py`, `python/sglang/srt/layers/quantization/w8a8_int8.py`；PR 正文摘要: - MoE quant-info refactor Extracts get_triton_quant_info() into each quantization method (FP8, INT8, WNA16, etc.) so FusedMoEWithLoRA correctly receives quantization scales/flag...。
- 实现要点: `python/sglang/srt/layers/quantization/fp8.py` modified +21/-20 (41 lines); hunks: -1487,6 +1487,26 @@ def create_moe_runner(; -1663,26 +1683,7 @@ def apply(; symbols: create_moe_runner, get_triton_quant_info, apply，涉及 `create_moe_runner, get_triton_quant_info, apply`；`python/sglang/srt/layers/quantization/moe_wna16.py` modified +13/-11 (24 lines); hunks: -364,19 +364,10 @@ def create_moe_runner(; -387,6 +378,17 @@ def apply(; symbols: create_moe_runner, apply, get_triton_quant_info，涉及 `create_moe_runner, apply, get_triton_quant_info`；`python/sglang/srt/layers/quantization/w8a8_int8.py` modified +13/-10 (23 lines); hunks: -331,6 +331,18 @@ def create_moe_runner(; -365,14 +377,5 @@ def apply(; symbols: create_moe_runner, get_triton_quant_info, apply，涉及 `create_moe_runner, get_triton_quant_info, apply`；`python/sglang/srt/layers/quantization/blockwise_int8.py` modified +10/-7 (17 lines); hunks: -360,13 +360,8 @@ def create_moe_runner(; -377,4 +372,12 @@ def apply(; symbols: create_moe_runner, apply, get_triton_quant_info，涉及 `create_moe_runner, apply, get_triton_quant_info`。
- 代码 diff 细节:
...
- 验证与风险: diff 自带测试面 `test/registered/lora/test_lora_deepseek_v3_base_logprob_diff.py`；如果继续改同一模型，优先复跑这些测试并补一个最小 launch/accuracy smoke。
...
- 反查来源: 保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 5 个文件，+91/-12，可读 patch 177 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[Reland] DeepSeek-R1-0528-w4a8: DeepEP Low Latency Dispatch Adopts FP8 Communication」；模型线: DeepSeek V3/R1；类别: 性能/后端优化；主要 diff: `python/sglang/srt/layers/moe/ep_moe/kernels.py`, `python/sglang/srt/layers/moe/cutlass_w4a8_moe.py`, `python/sglang/srt/layers/moe/ep_moe/layer.py`；PR 正文摘要: profiling: deepseek-R1-0528: deepseek-R1-0528-w4afp8: 1.When DeepEP is enabled, the communication latency of the DeepSeek-R1-0508-W4AFP8 model is twice that of the DeepSeek-R1-0...。
...
- 动机: 标题「[CPU] expand the interface of shared_expert without scaling factor」；模型线: DeepSeek V3/R1；类别: 模型实现调整；主要 diff: `sgl-kernel/csrc/cpu/moe_int4.cpp`, `sgl-kernel/csrc/cpu/moe.h`, `sgl-kernel/csrc/cpu/moe.cpp`；PR 正文摘要: This patch expands `shared_expert` kernel when scaling factor is None, which maps exactly the behavior of MoE when expert equals to one. Also simplifies moe kernel source codes...。
...
- 反查来源: `git log --name-only -- <model-files>` 反查到 `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mha.py`, `python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`, `python/sglang/srt/models/deepseek_common/utils.py`, `python/sglang/srt/models/deepseek_v2.py`；关联提交 `b35213be11c7`；保留自原 history/skill 显式引用
- 代码 diff 已读范围: GitHub Pull Request files API 返回 27 个文件，+184/-44，可读 patch 795 行；本卡优先审计模型相关文件和高变更量文件。
- 动机: 标题「[MUSA][16/N] Add MUSA backend support for layers and DeepSeek models (V2/V3/R1)」；模型线: DeepSeek V3/R1；类别: 模型支持/运行时入口；主要 diff: `python/sglang/srt/layers/deep_gemm_wrapper/compile_utils.py`, `python/sglang/srt/models/deepseek_v2.py`, `python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py`；PR 正文摘要: Enable SGLang to run DeepSeek models on Moore Threads MUSA GPUs. This PR adds MUSA backend support across the inference stack, including layers, quantization, MoE, attention, sp...。
- 实现要点: `python/sglang/srt/layers/deep_gemm_wrapper/compile_utils.py` modified +14/-3 (17 lines); hunks: -1,6 +1,6; -14,10 +14,12; symbols: execute, deep_gemm_execution_hook, _deep_gemm_execution_hook，涉及 `execute, deep_gemm_execution_hook, _deep_gemm_execution_hook`；`python/sglang/srt/models/deepseek_v2.py` modified +13/-4 (17 lines); hunks: -141,6 +141,7; -182,6 +183,8; symbols: forward_normal_dual_stream, _post_combine_hook, __init__, determine_num_fused_shared_experts，涉及 `forward_normal_dual_stream, _post_combine_hook, __init__`；`python/sglang/srt/models/deepseek_common/deepseek_weight_loader.py` modified +10/-1 (11 lines); hunks: -49,6 +49,7; -498,7 +499,7 @@ def post_load_weights(; symbols: post_load_weights，涉及 `post_load_weights`；`python/sglang/srt/models/deepseek_common/attention_forward_methods/forward_mla.py` modified +6/-0 (6 lines); hunks: -21,6 +21,7; -553,6 +554,11 @@ def forward_absorb_core(; symbols: forward_absorb_core，涉及 `forward_absorb_core`。
- 代码 diff 细节:
